...
KST = d2.KST
D4_VERSION = "kr-d4-loss-router-brake-fast-v2"
...
def run_kr_d4_loss_router_replay(
    ...
    if positions:
        for symbol, pos in list(positions.items()):
            frame = target_frames.get(symbol)
            ref_price = (
                _price_cached(frame, date_text, end, symbol)
...
def _job(
    ...
    try:
        ...
        # FAST screening: run only the two decision-relevant final variants.
        # Intermediate decomposition can be run later only if the final result is promising.
        modes = ["LOSS_ROUTER", "ROUTER_BRAKE"]
        ...
        _state(
            ...
            result_ready=False,
            error_days=0,
            message="D4 FAST · cached CONTROL + LOSS_ROUTER + ROUTER_BRAKE 비교 시작",
            last_error="",
        )
        ...
        # Full D2 CONTROL was already day-by-day parity-proven in D3.
        # FAST v2 reuses that canonical cache and runs only a small CONTROL spot-audit
        # to catch accidental engine drift without paying for 147 extra replays.
        ...
        sentinel_dates = {d for d in sentinel_candidates if d in set(available_dates)}
            ...
            if isinstance(cached, dict) and cached.get("version") == D4_VERSION:
                ...
                try:
                    ...
                    if date_text in sentinel_dates:
                        audit = run_kr_d4_loss_router_replay(
                            date_text=date_text,
                            codes=codes,
                            config=frozen_config,
                            mode="CONTROL",
                    ...
                    row = {
                        ...
                        **{mode: _pack(outputs[mode]) for mode in modes},
                    }
        ...
        router = _aggregate(
            "LOSS_ROUTER · 손실형 진입 + RED장 선택형 라우팅",
            "LOSS_ROUTER",
            rows,
        )
        brake = _aggregate(
        ...
        payload = {
            "ok": True,
            "version": D4_VERSION,
            "mode": "PATH_CONSISTENT_D4_LOSS_ROUTER_FAST_SCREEN",
            "read_only": True,
            "fast_screening": True,
            "executed_variants": ["LOSS_ROUTER", "ROUTER_BRAKE"],
            "deferred_decomposition": ["LOSS_TRAPS", "PG2_REARM"],
            ...
            "parity": {
                ...
                "rule": "FAST v2 reuses the already day-by-day parity-proven frozen D-v2 cache and spot-replays CONTROL on sentinel dates; full 147-day CONTROL can be rerun later for final audit.",
            },
            "variants": variants,
            ...
            "rules": {
                "CONTROL": "already parity-proven frozen D-v2 daily cache; sentinel CONTROL spot-audit only",
                "LOSS_TRAPS": "implemented inside LOSS_ROUTER; separate decomposition deferred in FAST screen",
                "PG2_REARM": "implemented inside LOSS_ROUTER; separate decomposition deferred in FAST screen",
            ...
            "important_limit": (
                ...
                "FAST screening runs only the two final variants first; if promising, run the deferred decomposition/full CONTROL audit. This is strategy-development validation, not a promise of live performance."
            ),
        }
        ...
        _state(
            ...
            message="D4 FAST LOSS ROUTER + BAD DAY BRAKE 검증 완료",
            last_error="",
        )
    except Exception as exc:
        _state(
