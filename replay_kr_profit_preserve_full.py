...
"""Read-only D-v2 profit-preserving loss shield full-engine replay.

The goal is not to kill upside by banning trades aggressively. Instead, the shield
keeps the original D-v2 entry universe and profit engine, while reducing exposure
when a setup is risky, trimming failing BUY1 positions early, gently protecting a
small pre-guard zone, and requiring a PG2 name to prove strength before full-size
re-entry.

Modes
- CONTROL: exact D-v2 behavior; used for day-by-day parity validation.
- GENTLE: adaptive 50%/25% sizing + partial BUY1 failure trim + PG2 graduated re-arm
          + overheated OPEN_STRONG starts at 25% and may OPEN_CONFIRM back to 50%.
- FULL: GENTLE + PRE_GUARD + post-stop next-entry size reduction until a winner resets it.

No real orders are sent. The long-backtest module supplies only the already-cached
Railway KIS 1-minute bars.
"""
...
KST = d2.KST
PROFIT_PRESERVE_VERSION = "kr-d2-profit-preserve-full-engine-v1"

_THREAD: threading.Thread | None = None
_LOCK = threading.RLock()

# The expensive TOP5 reconstruction is identical for CONTROL/GENTLE/FULL.
...
def run_kr_profit_preserve_replay(
    date_text: str,
    ...
    if mode not in {"CONTROL", "GENTLE", "FULL"}:
        raise ValueError("mode must be CONTROL, GENTLE, or FULL")
    shield = mode != "CONTROL"
    full = mode == "FULL"
    ...
    while now <= end:
        ...
        for symbol, pos in list(positions.items()):
            ...
            # GENTLE/FULL: normal BUY1 gets a two-stage failure defense instead of
            # waiting passively for the original -3% stop.
            is_normal_buy1 = str(pos.get("entry_action", "")) == "BUY1"
            if shield and is_normal_buy1:
                hold_n = _hold_minutes(pos, now)

                # A fast -1.8% failure exits fully. This is intentionally the same
                # emergency line already proven in OPEN DEFENSE.
                if pnl <= -1.80:
                    fill = replay_kr._fill_price(c_cfg, "SELL", ref_price)
                    realized = (fill - avg) * qty
                    add_event(
                        now, symbol, "NORMAL_EARLY_EMERGENCY_EXIT", "SELL", qty,
                        ref_price, fill,
                        f"일반 BUY1 급락 비상청산 · pnl {pnl:.2f}% <= -1.80%",
                        pnl, realized, vwap_gap=vg,
                    )
                ...
                if (
                    hold_n >= 3.0
                    and pnl <= -1.0
                    and not bool(pos.get("early_fail_reduced", False))
                ):
                    fail = _fail_signals(cfg, state, vg)
                    if len(fail) >= 2:
                        if qty <= 1:
                            diagnostic["normal_early_partial_small_qty_skips"] += 1
                            pos["early_fail_reduced"] = True
                        else:
                            ...
                            add_event(
                                now, symbol, "NORMAL_EARLY_PARTIAL_EXIT", "SELL", sell_qty,
                                ref_price, fill,
                                f"일반 BUY1 조기실패 절반축소 · 보유 {hold_n:.1f}분 · "
                                f"pnl {pnl:.2f}% · " + ",".join(fail),
                                pnl, realized, vwap_gap=vg,
                            )
                            pos["qty"] = qty - sell_qty
                            pos["early_fail_reduced"] = True
                            diagnostic["normal_early_partial_exits"] += 1
                            continue

                # FULL-only PRE_GUARD: if a trade almost reached the existing guard
                # trigger (+1.2%) but rolls all the way back to flat while losing VWAP
                # and short momentum, trim only ~25%, never the full position.
                if (
                    full
                    and not bool(pos.get("take1_sent", False))
                    and not bool(pos.get("pre_guard_sent", False))
                    and not bool(pos.get("early_fail_reduced", False))
                    and 0.80 <= peak < float(cfg.profit_guard_trigger_pct)
                    and pnl <= 0.15
                    and vg < 0.0
                    and state["ret3"] < 0.0
                ):
                    if qty <= 1:
                        diagnostic["pre_guard_small_qty_skips"] += 1
                        pos["pre_guard_sent"] = True
                    else:
                        ...
                        realized = (fill - avg) * sell_qty
                        add_event(
                            now, symbol, "PRE_GUARD", "SELL", sell_qty,
                            ref_price, fill,
                            f"PRE_GUARD 25%축소 · 최고 +{peak:.2f}% → 현재 {pnl:.2f}% · "
                            f"VWAP {vg:+.2f}% · 3분 {state['ret3']:+.2f}%",
                            pnl, realized, vwap_gap=vg,
                        )
                        pos["qty"] = qty - sell_qty
                        pos["pre_guard_sent"] = True
                        diagnostic["pre_guard_exits"] += 1
                        continue
                    ...
                    if shield:
                        ...
                        if disarmed is not None:
                            ...
                            if rearmed:
                                ...
                                risk_reasons.append("PG2_NOT_REARMED")

                        # Preserve participation: risky eligible BUY1s are generally
                        # reduced to 25%, not banned.
                        if vg < 0.0:
                            risk_reasons.append("VWAP_BELOW")
                        if vg > 2.50:
                            risk_reasons.append("VWAP_OVERHEAT")

                        if sec >= late_1045_sec:
                            late_a = bool(
                                state["rank"] <= 3
                                and 1.0 <= vg <= 2.0
                                and state["ret3"] > 0.0
                            ...
                            if not late_a:
                                risk_reasons.append("LATE_NOT_A")
                        if sec >= late_1300_sec:
                            risk_reasons.append("AFTER_1300")

                        if full and risk_reduced_after_stop and not _is_a_plus(state, vg):
                            risk_reasons.append("POST_STOP_RISK")

                        if risk_reasons:
                            target_amount = defense_amount
                            diagnostic["adaptive_25pct_entries"] += 1
                            if "VWAP_BELOW" in risk_reasons:
                                diagnostic["adaptive_vwap_below_entries"] += 1
                            if "VWAP_OVERHEAT" in risk_reasons:
                                diagnostic["adaptive_overheat_entries"] += 1
                            if "LATE_NOT_A" in risk_reasons or "AFTER_1300" in risk_reasons:
                                diagnostic["adaptive_late_entries"] += 1
                            if "POST_STOP_RISK" in risk_reasons:
                                diagnostic["adaptive_post_stop_entries"] += 1
                            if "PG2_NOT_REARMED" in risk_reasons:
                                diagnostic["pg2_probe_entries"] += 1
    ...
    return {
        "ok": True,
        "version": PROFIT_PRESERVE_VERSION,
        "date": date_text,
        ...
        "rules": {
            ...
            "gentle": {
                "risky_BUY1": "eligible trades stay alive at 25% rather than being broadly banned",
                "late_rule": "after 10:45 only A-location keeps 50%; after 13:00 participation remains at 25%",
                "normal_early_fail": "-1.0% + 2 failure signals => ~50% trim; -1.8% => full emergency exit",
                "PG2": "10m hard lock; after 10m 25% probe until TOP3+VWAP+3m+5m re-arm; re-armed => 50%",
                "open_strong_overheat": ">=+2.5% VWAP starts 25% and may OPEN_CONFIRM to 50%",
            },
            "full_extra": {
                "pre_guard": "+0.8~+1.2% peak then flat+below VWAP+ret3 negative => ~25% trim",
                "post_stop": "next ordinary BUY1 uses 25% until a winning exit; A+ remains 50%",
            },
        },
        "assumptions": {
            "real_orders": False,
            ...
            "fees_taxes": "별도 미포함",
            "purpose": "profit-preserving D-v3 loss reduction full-engine validation",
        },
    }
...
def _paths():
    base = _base()
    root = base.ROOT / "profit_preserve_full_engine"
    day_dir = root / "daily"
...
def _state(**updates) -> dict:
    ...
    with _LOCK:
        ...
        cur.update(updates)
        cur.setdefault("version", PROFIT_PRESERVE_VERSION)
...
def _job(result: dict) -> None:
    ...
    try:
        ...
        _state(
            status="running",
            phase="PROFIT_PRESERVE_FULL_ENGINE",
            started_at=datetime.now(KST).isoformat(timespec="seconds"),
            total_days=len(daily_base),
            completed_days=0,
            progress_pct=0.0,
            result_ready=False,
            message="CONTROL / GENTLE / FULL 수익보존형 손실방어 전체엔진 비교 시작",
            last_error="",
        )
        ...
        for idx, base_row in enumerate(daily_base, start=1):
            ...
            _state(
                ...
                message=f"수익보존형 전체엔진 {idx}/{len(daily_base)} · {date_text}",
            )

            day_path = _day_path(date_text)
            cached = base._load_gzip_json(day_path, None)
            if isinstance(cached, dict) and cached.get("version") == PROFIT_PRESERVE_VERSION:
                row = cached
            else:
                try:
                    control = run_kr_profit_preserve_replay(
                        date_text=date_text, codes=codes, config=cfg, mode="CONTROL"
                    )
                    gentle = run_kr_profit_preserve_replay(
                        date_text=date_text, codes=codes, config=cfg, mode="GENTLE"
                    )
                    full = run_kr_profit_preserve_replay(
                        date_text=date_text, codes=codes, config=cfg, mode="FULL"
                    )
                    row = {
                        "version": PROFIT_PRESERVE_VERSION,
                        "date": date_text,
                        ...
                        "GENTLE_SHIELD": _pack(gentle),
                        "FULL_SHIELD": _pack(full),
                    }
                    row["parity_delta_KRW"] = (
                        int(row["CONTROL_D2"]["pnl_KRW"]) - int(row["cached_D2_KRW"])
                    )
                    for key in ("GENTLE_SHIELD", "FULL_SHIELD"):
            ...
            _state(
                ...
                message=f"수익보존형 전체엔진 {idx}/{len(daily_base)} 완료",
            )

        control = _aggregate("D-v2 원본 전체엔진", "CONTROL_D2", rows)
        gentle = _aggregate(
            "GENTLE · 25% 위험축소 + 부분실패청산 + PG2 재무장 + 장초반 과열 25%",
            "GENTLE_SHIELD", rows,
        )
        full = _aggregate(
            "FULL · GENTLE + PRE_GUARD + STOP후 다음진입 위험축소",
            "FULL_SHIELD", rows,
        )
        for v in (gentle, full):
        ...
        preservation = {
            "GENTLE_SHIELD": _profit_preservation(rows, "GENTLE_SHIELD"),
            "FULL_SHIELD": _profit_preservation(rows, "FULL_SHIELD"),
        }
        monthly = {
            "GENTLE_SHIELD": _monthly(rows, "GENTLE_SHIELD"),
            "FULL_SHIELD": _monthly(rows, "FULL_SHIELD"),
        }
        ...
        if parity_ok:
            for v in (gentle, full):
                ...
                if pp["passes_90pct_profit_preservation"] and v["total_KRW"] > control["total_KRW"]:
                    candidates.append(v)
        best = max(candidates, key=lambda x: int(x["total_KRW"])) if candidates else None

        target_improvement = 1_000_000
        stretch_total = -500_000
        best_delta = int((best or {}).get("delta_vs_control_KRW", 0) or 0)

        diffs_g = [day_delta(r, "GENTLE_SHIELD") for r in rows]
        diffs_f = [day_delta(r, "FULL_SHIELD") for r in rows]

        payload = {
            "ok": True,
            "version": PROFIT_PRESERVE_VERSION,
            "mode": "PATH_CONSISTENT_PROFIT_PRESERVE_A_B_C",
            "read_only": True,
            ...
            "variants": [control, gentle, full],
            "profit_preservation": preservation,
            "monthly": monthly,
            "best_if_parity_and_90pct_profit_preserved": best,
            "target": {
            ...
            "top_gentle_improved_days": sorted(diffs_g, key=lambda x: x["delta_KRW"], reverse=True)[:10],
            "top_gentle_worsened_days": sorted(diffs_g, key=lambda x: x["delta_KRW"])[:10],
            "top_full_improved_days": sorted(diffs_f, key=lambda x: x["delta_KRW"], reverse=True)[:10],
            "top_full_worsened_days": sorted(diffs_f, key=lambda x: x["delta_KRW"])[:10],
            "rules": {
                "profit_engine": "TAKE1 +3%, TAKE2 +5%, original PROFIT_GUARD unchanged",
                "GENTLE": [
                    "risky eligible BUY1 => generally 25%, not blanket ban",
                    "after 10:45 only A-location retains 50%; after 13:00 still 25% participation",
                    "normal BUY1 -1% + 2 failures => partial trim; -1.8% emergency full exit",
                    "PG2 10m hard lock, then 25% probe until TOP3+VWAP+3m+5m re-arm",
                    "OPEN_STRONG >=+2.5% VWAP => 25% start, OPEN_CONFIRM can restore 50%",
                ],
                "FULL_EXTRA": [
                    "PRE_GUARD: +0.8~+1.2% peak then flat/below VWAP/ret3 negative => ~25% trim",
                    "after normal BUY1 final stop/emergency, ordinary next BUY1 25% until winner; A+ stays 50%",
                ],
            },
            "daily": rows,
            "important_limit": (
        ...
        _state(
            ...
            message="수익보존형 손실방어 전체엔진 검증 오류",
        )


def ensure_profit_preserve_started(result: dict) -> dict:
    ...
    if (
        ...
        and existing.get("version") == PROFIT_PRESERVE_VERSION
    ...
    if runner_thread is not None and runner_thread.is_alive():
        ...
        state.update({
            ...
            "phase": "WAIT",
            "result_ready": False,
            "message": "기존 Runner 전체엔진 검증 종료 후 수익보존형 손실방어를 시작합니다.",
            "started": False,
        })
        return state
    ...
    with _LOCK:
        ...
        _THREAD = threading.Thread(
            ...
            args=(dict(result),),
            daemon=True,
            name="kr-d2-profit-preserve-full-engine",
        )
        _THREAD.start()
        state = _public_state()
        state["started"] = True
