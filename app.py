...
with st.sidebar:
    ...
    # AI 시장판단은 항상 사용 (화면에서 ON/OFF 버튼 제거)
    ai_filter_on = True
...
# Railway worker의 실제 최근 결과를 우선 표시합니다.
worker_snapshot = load_worker_status()
remote_cycle_key = (
    "kr_last_result"
    if market == "국내"
    else "us_last_result"
...
if worker_snapshot:
    cycle = worker_snapshot.get(remote_cycle_key)
    if cycle:
        cycle_source = "Railway worker"

# Railway 결과가 아직 없을 때만 앱에서 수동 실행한 결과를 사용합니다.
if not cycle:
    cycle = st.session_state.get(cycle_key)
    if cycle:
        cycle_source = "앱 수동 실행"

if not cycle:
    st.caption(
        "Railway worker에서 아직 완료된 자동매매 사이클 결과가 없습니다."
    )

else:
    if cycle_source:
        st.caption(f"결과 출처: {cycle_source}")
