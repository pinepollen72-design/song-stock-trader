...
st.caption(
    "앱은 후보·TOP5·판단 결과를 확인하는 화면입니다. "
    "실제 모의 자동주문은 Render worker가 계속 자동으로 담당합니다."
)
...
# =========================================================
st.divider()
st.header("🔎 후보·판단 수동 새로고침")

st.write(
    "이 버튼은 화면의 후보와 판단 결과만 즉시 다시 계산합니다."
)
...
if st.button(
    ...
    try:
        if market == "국내":
            ...
            st.info(
                "🧪 앱에서는 판단만 완료했습니다. "
                "실제 모의주문 전송은 Render worker가 담당합니다."
            )
...
st.info(
    "🛡️ 앱 화면은 항상 판단 전용입니다. "
    "실제 모의주문 전송은 Render worker 한쪽만 담당합니다."
)
