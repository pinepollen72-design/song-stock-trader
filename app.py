...
WORKER_STATUS_URL = (
    _secret("WORKER_STATUS_URL")
    or _secret("SONG_WORKER_STATUS_URL")
    or DEFAULT_WORKER_STATUS_URL
).rstrip("/")

# Railway의 /status 보호용 비밀토큰.
# 값 자체는 GitHub 코드에 넣지 않고 Streamlit Secrets에만 저장합니다.
STATUS_TOKEN = _secret("SONG_STATUS_TOKEN")
...
def fetch_status() -> dict:
    if not STATUS_TOKEN:
        st.error("SONG_STATUS_TOKEN이 Streamlit Secrets에 설정되지 않았습니다.")
        return {}
    ...
    try:
        r = requests.get(
            ...
            headers={
                ...
                "Pragma": "no-cache",
                "X-Song-Status-Token": STATUS_TOKEN,
            },
        )
        r.raise_for_status()
        data = r.json()
