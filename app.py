...
st.caption(
    "TOP5 선정 → Railway Worker 주문 → 한국투자 실제 잔고 확인 "
...
def fetch_status() -> dict:
    try:
        ...
        if not isinstance(data, dict):
            raise ValueError(
                "Worker /status 응답이 JSON 객체가 아닙니다."
            )
        return data
    except Exception as e:
        st.error(
            f"Railway Worker 상태조회 실패: "
            f"{type(e).__name__}: {e}"
        )
        st.caption(
            f"조회 주소: {WORKER_STATUS_URL}/status"
        )
        return {}
...
if not result:
    ...
    buy = sum(
        1
        for a in actions
        if str(a.get("action", "")).startswith("BUY")
        and a.get("status") in ("FILLED", "ORDERED")
    )
    sell = sum(
        1
        for a in actions
        if not str(a.get("action", "")).startswith("BUY")
        and a.get("status") in ("FILLED", "ORDERED")
    )
    fail = sum(
        1
        for a in actions
        if a.get("status") in ("REJECT", "ERROR")
    )
...
if journal.empty:
    st.info(
        "새 V2 Worker가 한국투자에 보낸 주문 기록이 아직 없습니다. "
        "장 종료 후 새로 배포한 경우 정상입니다."
    )
    st.caption(
