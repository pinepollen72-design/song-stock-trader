...
if ai_filter_on:
    if leader_for_ai.empty:
        ...
        if ai_result is None:
            st.caption("위 버튼을 누르면 이곳에 AI 시장요약과 종목별 판정이 표시됩니다.")
            ...
            try:
                ...
                st.error(f"AI 결과 병합 실패: {type(e).__name__}: {e}")
else:
...
if market != "국내":
    ...
    leader_for_trade = st.session_state.get("leader_df_kr", pd.DataFrame())

    if ai_filter_on:
        ai_filtered_for_trade = st.session_state.get("ai_filtered_국내", pd.DataFrame())
