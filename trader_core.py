...
class KISClient:
    ...
    def get_token(self) -> str:
        ...
        self.token_file.write_text(json.dumps({"token": token, "expires_at": expires_at}), encoding="utf-8")
        return token
    ...
    def domestic_intraday_minutes(
        ...
        """
        KIS 주식당일분봉조회(v1_국내주식-022).

        실전/모의 모두 TR_ID FHKST03010200을 사용하며 한 번 호출에 최대 30건의
        당일 분봉을 반환한다. 최근 3/5/10/20분 모멘텀 확인용으로 사용한다.
        """
        ...
        if not reference_time:
            reference_time = datetime.now(ZoneInfo("Asia/Seoul")).strftime("%H%M%S")
        reference_time = "".join(ch for ch in str(reference_time) if ch.isdigit())[:6].zfill(6)

        market_division = str(market_division or "J").upper()
