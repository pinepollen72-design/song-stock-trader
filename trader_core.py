...
class KISClient:
    def __init__(self, settings: Settings, env: str = "demo"):
        ...
        # KIS REST API는 CANO 8자리 + ACNT_PRDT_CD 2자리를 따로 요구합니다.
        # 환경변수에 50201218-01처럼 전체 계좌번호가 들어와도 자동으로 분리합니다.
        ...
        # KIS 모의투자는 REST 호출 제한이 낮습니다.
        # 모든 REST 조회/주문 사이에 최소 간격을 두어 EGW00201을 예방합니다.
    ...
    def get(self, path: str, tr_id: str, params: Dict[str, Any]) -> Dict[str, Any]:
        # 조회는 rate-limit 응답일 때만 안전하게 재시도합니다.
        ...
        for attempt in range(max_attempts):
            ...
            # 모의투자 제한에 걸렸다면 다음 재시도 전 더 여유를 둡니다.
    ...
    def post(self, path: str, tr_id: str, body: Dict[str, Any]) -> Dict[str, Any]:
        # 주문 POST는 중복 주문 위험 때문에 자동 재시도하지 않습니다.
        self._rest_wait()
        r = requests.post(
            f"{self.base_url}{path}",
            headers=self._headers(tr_id),
    ...
    def domestic_buying_power(
        ...
        자동매매는 이 값을 먼저 확인한 뒤 주문 수량을 줄여서
        모의계좌의 주문가능금액 부족 오류를 예방합니다.
        """
    ...
    def overseas_present_balance_us(
        ...
        """KIS 해외주식 체결기준현재잔고. 모의 TR: VTRP6504R"""
    ...
    def overseas_buying_power_us(
        ...
        """
        미국주식 매수가능금액/수량 조회.
        KIS 해외주식 매수가능금액조회(inquire-psamount)를 사용합니다.

        주요 output:
        - ovrs_ord_psbl_amt: 해외주문가능금액(외화 기준)
        - max_ord_psbl_qty: 최대주문가능수량
        - ord_psbl_frcr_amt: 주문가능외화금액
        - frcr_ord_psbl_amt1: 통합 주문가능 외화금액
        """
...
def parse_domestic_holdings(
    ...
    def numeric_series(col_name, default=0.0):
        if not col_name:
            return default
    ...
    out["보유수량"] = numeric_series(qty, 0).astype(int) if qty else 0
    out["평균매입가"] = numeric_series(avg, 0.0) if avg else 0.0
    out["현재가"] = numeric_series(cur, 0.0) if cur else 0.0
