...
class KISClient:
    ...
    def get_token(self, force: bool = False) -> str:
        if self._token and not force and time.time() - self._token_time < 23 * 3600:
            return self._token
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if r.status_code == 403:
                    raise RuntimeError(
                        "KIS 토큰 403 Forbidden. 모의 APP KEY/SECRET, API 사용상태, "
                        "동시 토큰발급 여부를 확인하세요. 응답=" + r.text[:500]
                    )
                ...
                if attempt < self.max_retries:
                    ...
                    continue
                raise RuntimeError(
                    f"KIS 토큰 네트워크 오류({attempt}회 시도): {type(e).__name__}: {e}"
                ) from e
