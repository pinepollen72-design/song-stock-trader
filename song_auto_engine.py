...
def run_domestic_cycle(
    ...
    if execute_orders and not (dtime(9, 0) <= now.time() < dtime(15, 30)):
        ...
        result["order_gate_message"] = (
            "분석은 08:30~16:00, 실제 주문 전송은 09:00~15:30에만 허용합니다."
        )
    ...
    for symbol, pos in list(state["positions"].items()):
        ...
        # 완화/예외 진입은 1차만
        if pos.get("leader_exception") or pos.get("demo_relaxed_entry"):
            continue
    ...
    for _, row in leader_df.iterrows():
        ...
        leader_exception = (
            ...
            and is_rank1
            and lead_score >= float(getattr(config, "leader_exception_min_lead_score", 75.0))
            and combined >= float(getattr(config, "leader_exception_min_combined_score", 60.0))
        )

        demo_relaxed_entry = (
            is_demo
            and bool(getattr(config, "demo_relaxed_entry_enabled", True))
            and combined >= float(getattr(config, "demo_min_combined_score", 40.0))
        )
        ...
        if normal_entry:
            entry_reason = f"정상 1차매수 · 종합점수 {combined:.1f} · {signal}"
            action_name = "BUY1"
            relaxed_flag = False
            ...
            entry_reason = f"모의완화 1차매수 · TOP5 · 종합점수 {combined:.1f} · {signal}"
            action_name = "BUY1_DEMO_RELAXED"
            relaxed_flag = True
            leader_exception_flag = False

        act = _place_order(
            client, state, symbol, "buy", qty, entry_reason, execute_orders
        )
    ...
    if not result["actions"]:
        if is_demo:
            result["message"] = (
                f"모의 완화기준 포함 신규매수 후보 없음 "
                f"(demo 기준 종합점수 {getattr(config, 'demo_min_combined_score', 40.0):.0f}+)"
            )
        else:
...
def _place_overseas_order(
    ...
    if not execute_orders:
        ...
        return {"status": "DRY", "qty": qty, "price": price}
...
def run_overseas_cycle(
    ...
    for symbol, pos in list(state["positions"].items()):
        ...
        if now.time() >= _clock(config.us_force_exit_time):
            act = _place_overseas_order(
                ...
                f"당일 강제청산 {config.us_force_exit_time} ET", execute_orders,
            )
            result["actions"].append({"symbol": symbol, "action": "FORCE_SELL", "pnl": pnl, **act})
            continue
    ...
    if not result["actions"]:
        result["message"] = "🟢 매수 후보 + 종합점수 기준을 통과한 미국 신규매수 후보 없음"
