...
class AutoConfig:
    ...
    def __post_init__(self):
        # Railway worker.py가 넘기는 V2 이름을
        # 기존 국내 엔진 내부 이름과 자동 동기화합니다.
...
def run_overseas_cycle(
    ...
    핵심 안전규칙:
    - 주문접수(ORDERED) 후 실제 KIS 잔고가 반영되기 전에는 같은 종목에
      어떤 추가 주문도 보내지 않는다.
    - pending_order는 체결 확인 전까지 당일 유지한다. 따라서 지정가가
      미체결 상태로 남아 있어도 다음 worker 사이클에서 신규매수가 반복되지 않는다.
    - 첫 매수/추가매수/익절/손절/강제청산 모두 같은 pending 보호를 적용한다.
    """
    state = load_us_state()
    now = _now_et()
    ...
    result["holdings"] = [
        {"symbol": s, **h} for s, h in holdings.items()
    ]

    # -----------------------------------------------------
    # 1. 기존 추적 종목 관리 + 미체결/잔고반영 대기 잠금
    # -----------------------------------------------------
    for symbol, pos in list(state["positions"].items()):
        actual = holdings.get(symbol, {})
        actual_qty = int(actual.get("qty", 0))
        ...
        if isinstance(pending, dict) and pending:
            ...
            expected_after_qty = int(
                pending.get("expected_after_qty", before_qty) or before_qty
            )
            ...
            if confirmed:
                ...
                if pending_stage in (1, 2, 3):
                    pos["buy_stage"] = max(
                        int(pos.get("buy_stage", 0) or 0),
                        pending_stage,
                    )
                ...
                _diag(
                    ...
                    (
                        ...
                        f"{pending.get('qty', 0)}주 · "
                        f"KIS 현재 확인수량 {actual_qty}주 · "
                        f"주문시각 {sent_at}. "
                        f"같은 종목 추가 주문 차단"
                    ),
                    before_qty=before_qty,
                ...
                # 가장 중요한 안전장치: 체결 확인 전에는 이 종목에 추가 주문 금지.
                continue

        # 첫 주문이 실제로 체결되기 전 상태인데 pending 정보가 없는 구버전 state라면
        # 당일에는 추적을 유지해 신규매수 반복을 막는다.
        if actual_qty <= 0:
            if execute_orders:
                ...
                _diag(
                    result,
                    symbol,
                    "WAIT_BALANCE_CONFIRMATION",
                    "주문 추적중이나 KIS 보유수량이 아직 0주라 추가 주문 차단",
                    after_qty=0,
                )
                continue
            # DRY 모드에서는 실제 주문이 없으므로 기존 동작과 같이 관리만 건너뜀.
            continue
        ...
        def _set_pending(action: str, side: str, order_qty: int, act: Dict[str, Any], stage: int = 0):
            if act.get("status") != "ORDERED":
                return
            if side == "buy":
                expected_after = actual_qty + int(order_qty)
            else:
                expected_after = max(0, actual_qty - int(order_qty))
            pos["pending_order"] = {
                "action": action,
                "side": side,
                "qty": int(order_qty),
                "before_qty": int(actual_qty),
                "expected_after_qty": int(expected_after),
                "stage": int(stage),
                "sent_at": now.isoformat(),
                "price": float(act.get("price", price) or price),
            }
        ...
        if stage == 2 and pnl >= config.add3_trigger_pct:
            ...
            if (
                ...
                _set_pending("BUY3", "buy", qty, act, stage=3)
                result["actions"].append(
                    {
                        "symbol": symbol,
                        "action": "BUY3",
    ...
    for _, row in leader_df.iterrows():
        ...
        # state에 있으면 실제 잔고 0이어도 '주문대기'로 간주하여 재주문 금지.
        ...
        reason = (
            ...
            f"사용가능 ${_us_stage_budget(config, 1):.2f} / {qty}주"
        )
        if signal:
            reason += f" · {signal}"
        ...
        if act.get("status") == "ORDERED":
            ...
            # DRY는 실제 주문이 아니므로 다음 사이클의 실주문을 막는 pending을 만들지 않는다.
            pass
