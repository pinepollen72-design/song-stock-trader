...
def _sync_us_budget_from_krw_local(config):
    ...
    return {
        ...
        "daily_budget_usd": float(getattr(config, "us_daily_budget_usd", 0) or 0),
        "per_stock_budget_usd": float(getattr(config, "us_per_stock_budget_usd", 0) or 0),
    }
...
def _us_time_kst_label(hhmm: str) -> str:
    h, m = [int(x) for x in hhmm.split(":")]
    now_et = datetime.now(ET)
    et_dt = now_et.replace(
        hour=h,
        minute=m,
        second=0,
        microsecond=0,
    ...
    if kst_dt.date() > et_dt.date():
        day_note = " 다음날"
    elif kst_dt.date() < et_dt.date():
        day_note = " 전날"
    else:
        day_note = ""

    return (
        f"{kst_dt.strftime('%H:%M')} KST{day_note} "
        f"({hhmm} ET)"
    )
...
with st.sidebar:
    ...
    ai_filter_on = st.toggle(
        ...
        value=False,
        help=(
            "켜면 대장주 TOP5 선정 후 최신 뉴스/이슈를 "
            "추가 확인합니다. AI가 꺼져 있으면 숫자 규칙만 사용합니다."
        ),
    )
...
_sync_us_budget_from_krw_local(cfg)
...
if market == "국내":
    ...
    us_prep = _us_time_kst_label("09:00")
    us_open = _us_time_kst_label("09:30")
    us_last = _us_time_kst_label("15:30")
    us_force = _us_time_kst_label("15:50")
    us_close = _us_time_kst_label("16:00")

    st.info(
        "🇺🇸 미국장 한국시간 기준\n\n"
        f"후보준비 {us_prep} · "
        f"주문시작 {us_open} · "
        f"신규/추가매수 종료 {us_last} · "
        f"강제청산 {us_force} · "
        f"정규장 종료 {us_close}"
    )
    st.info(
...
st.write(
    "이 버튼 하나가 아래 과정을 자동 실행합니다."
)

st.write(
    "후보 탐색 → 대장주 TOP5 → 기술분석 → "
    "AI 필터(선택) → 최종 후보 → 자동 주문전송"
)
...
if st.button(
    ...
    try:
        if market == "국내":
            ...
            st.success(
                f"✅ 1단계 완료: "
                f"{len(candidates)}개 후보 발견"
            )

            st.info(
                "② 대장주 TOP5와 기술점수를 계산합니다."
            )

            leader_df = (
                build_domestic_leader_df(
                    candidates
                )
            )
            ...
            if ai_filter_on:
                ...
                try:
                    ...
                    if (
                        "AI통과"
                        not in ai_filtered.columns
                    ):
                        st.warning(
                            "AI 통과 결과를 확인할 수 없습니다."
                        )
                        st.stop()
            ...
            if env == "demo":
                st.success(
                    "✅ 국내 모의 자동매매 주문 전송까지 완료했습니다."
                )
            else:
                st.success(
                    "🔴 국내 실전 자동매매 주문 전송까지 완료했습니다."
                )
            ...
            default_us = [
                ...
                "AMD","NFLX","PLTR","ORCL","CRM","ADBE","INTC","QCOM",
                "MU","AMAT","ARM","SMCI","JPM","BAC","GS","V","MA",
                "COIN","HOOD","UBER","ABNB","WMT","COST","HD","NKE",
                "DIS","BA","CAT","GE","LLY","UNH","XOM",
            ]
            ...
            if env == "demo":
                st.success(
                    "✅ 미국 모의 자동매매 주문 전송까지 완료했습니다."
                )
            else:
                st.success(
                    "🔴 미국 실전 자동매매 주문 전송까지 완료했습니다."
                )
...
# =========================================================
st.warning(
    "⚠️ 이 앱의 원터치 버튼은 실제 모의주문까지 전송합니다. "
    "Render worker도 WORKER_EXECUTE_ORDERS=true이면 중복주문 위험이 있으므로 "
    "앱으로 주문할 때는 worker 주문전송을 OFF로 두는 것을 권장합니다."
)
