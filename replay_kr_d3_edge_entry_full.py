...
"""Read-only D3 EDGE ENTRY full-engine replay.

The experiment changes *entry selection only*. The frozen D-v2 opening defense,
position sizing, STOP, TAKE1, TAKE2 and PROFIT_GUARD logic stay unchanged.

Modes
- CONTROL: exact frozen D-v2 behavior for day-by-day parity.
- MARKET: block ordinary BUY1 only when broad market breadth is clearly RED.
- BREAKOUT: ordinary BUY1 requires persistent TOP3 leadership plus a real 5-bar breakout.
- COMBO: both MARKET and BREAKOUT filters.

No real orders are sent. Only the already-cached Railway KIS 1-minute bars are read.
No future bars are used in any decision.
"""
...
KST = d2.KST
EDGE_VERSION = "kr-d3-edge-entry-full-engine-v1"

_THREAD: threading.Thread | None = None
_LOCK = threading.RLock()

# TOP5 and market-breadth reconstruction are shared across all four modes.
# Cache once per day/scan timestamp so the 147-day full-engine test stays practical.
_TOP5_CACHE_DATE = ""
...
def _bars_until_safe(frame: pd.DataFrame, date_text: str, now: pd.Timestamp) -> pd.DataFrame:
    try:
        bars = replay_kr._bars_until(frame, now, date_text)
        if bars is None:
            return pd.DataFrame()
        return bars
    except Exception:
...
def _market_snapshot_cached(
    ...
    """Broad-market state using only bars available at *now*.

    RED is intentionally conservative: at least two of three breadth/momentum
    warnings must be weak. MARKET mode blocks only RED, preserving NEUTRAL days.
    """
    global _MARKET_CACHE_DATE, _MARKET_CACHE
    if _MARKET_CACHE_DATE != date_text:
        _MARKET_CACHE_DATE = date_text
        _MARKET_CACHE = {}
    key = now.isoformat()
    ...
    for symbol, frame in target_frames.items():
        bars = _bars_until_safe(frame, date_text, now)
        if bars.empty:
            continue
        closes = pd.to_numeric(bars.get("Close", 0), errors="coerce").dropna()
        if len(closes) < 2:
            continue
        ...
        if price <= 0:
            continue
        total += 1
        vg = d2._vwap_gap_pct(frame, date_text, now, price)
        if vg >= 0.0:
            above_vwap += 1
        if len(closes) >= 6:
            base = float(closes.iloc[-6])
            if base > 0 and (price / base - 1.0) * 100.0 > 0.0:
                ret5_positive += 1

    above_pct = 100.0 * above_vwap / total if total else 50.0
    ret5_pct = 100.0 * ret5_positive / total if total else 50.0

    top_ret3: list[float] = []
    top_ret5: list[float] = []
    if latest_top5 is not None and not latest_top5.empty:
        for _, row in latest_top5.iterrows():
            top_ret3.append(d2._safe_float(row.get("최근3분수익률", 0)))
            top_ret5.append(d2._safe_float(row.get("최근5분수익률", 0)))
    med3 = float(pd.Series(top_ret3).median()) if top_ret3 else 0.0
    med5 = float(pd.Series(top_ret5).median()) if top_ret5 else 0.0

    weak_flags = [above_pct < 45.0, ret5_pct < 45.0, med5 < 0.20]
    strong_flags = [above_pct >= 55.0, ret5_pct >= 52.0, med5 >= 0.35 and med3 >= 0.15]
    if total >= 20 and sum(bool(x) for x in weak_flags) >= 2:
        regime = "RED"
    elif total >= 20 and sum(bool(x) for x in strong_flags) >= 2:
        regime = "GREEN"
    else:
        regime = "NEUTRAL"
    ...
    out = {
        ...
        "above_vwap_pct": round(above_pct, 2),
        "ret5_positive_pct": round(ret5_pct, 2),
        "top5_median_ret3_pct": round(med3, 3),
        "top5_median_ret5_pct": round(med5, 3),
    }
    _MARKET_CACHE[key] = dict(out)
    return out
...
def _breakout_allowed(
    ...
    """Persistent TOP3 + close above previous five completed-bar highs."""
    if leader_streak < 2:
        return False, "LEADER_NOT_PERSISTENT"
    if state["rank"] > 3 or state["weak"] or not state["signal"]:
        return False, "LEADER_NOT_TOP3"
    if vwap_gap < 0.0 or state["ret3"] <= 0.0 or state["ret5"] <= 0.0:
        return False, "MOMENTUM_NOT_CONFIRMED"

    bars = _bars_until_safe(frame, date_text, now)
    if len(bars) < 6:
        return False, "INSUFFICIENT_BARS"
    prior = bars.iloc[:-1].tail(5)
    highs = pd.to_numeric(prior.get("High", 0), errors="coerce").dropna()
    if highs.empty:
        return False, "NO_PIVOT"
    pivot = float(highs.max())
    if pivot <= 0:
        return False, "NO_PIVOT"
    # Require the latest completed close to clear the prior five-bar high.
    if float(ref_price) <= pivot:
        return False, f"WAIT_BREAKOUT<={pivot:.2f}"
    return True, f"BREAKOUT>{pivot:.2f}"
...
def run_kr_d3_edge_entry_replay(
    date_text: str,
    codes: Iterable[str] | None = None,
    config: d2.OpenDefenseConfig | None = None,
    mode: str = "CONTROL",
) -> dict:
    """Run one day with entry-only D3 experiments.

    CONTROL is intentionally the same path as the previously parity-proven D-v2
    full engine. The three variants alter ordinary BUY1 permission only.
    """
    cfg = config or d2.OpenDefenseConfig()
    mode = str(mode or "CONTROL").upper().strip()
    if mode not in {"CONTROL", "MARKET", "BREAKOUT", "COMBO"}:
        raise ValueError("mode must be CONTROL, MARKET, BREAKOUT, or COMBO")
    use_market = mode in {"MARKET", "COMBO"}
    use_breakout = mode in {"BREAKOUT", "COMBO"}
    # Hard-disable all former SURGICAL interventions. This keeps exits, opening
    # behavior and ordinary 50% sizing frozen to D-v2 in every EDGE mode.
    shield = False
    strict95 = False
    balanced90 = False
    full = False
    ...
    red_scan_seen: set[str] = set()

    # Former PG2 state is inert in EDGE modes; kept only so the parity-proven engine structure stays intact.
    ...
    diagnostic = {
        ...
        "risk_state_resets": 0,
        # D3 EDGE ENTRY diagnostics
        "market_red_scans": 0,
        "market_blocked_signals": 0,
        "leader_persistence_waits": 0,
        "breakout_waits": 0,
        "breakout_entries": 0,
        "edge_entries_market": 0,
        "edge_entries_breakout": 0,
        "edge_entries_combo": 0,
    }
    ...
    while now <= end:
        ...
        if last_scan is None or (now - last_scan).total_seconds() >= int(cfg.scan_seconds):
            ...
            if use_market:
                last_market_snapshot = _market_snapshot_cached(
                    target_frames, date_text, now, latest_top5
                )
                if last_market_snapshot.get("regime") == "RED":
                    k = now.isoformat()
                    if k not in red_scan_seen:
                        red_scan_seen.add(k)
                        diagnostic["market_red_scans"] += 1
        ...
        if (
            ...
            for _, row in latest_top5.iterrows():
                ...
                if defense_now:
                    ...
                    # ---------------- D3 EDGE ENTRY: entry permission only ----------------
                    if use_market:
                        snap = last_market_snapshot or _market_snapshot_cached(
                            target_frames, date_text, now, latest_top5
                        )
                        if snap.get("regime") == "RED":
                            diagnostic["market_blocked_signals"] += 1
                            continue

                    if use_breakout:
                        streak = int(leader_streaks.get(symbol, 0))
                        ok_break, break_reason = _breakout_allowed(
                            frame, date_text, now, ref_price, state, streak, vg
                        )
                        if not ok_break:
                            if streak < 2 or state["rank"] > 3:
                                diagnostic["leader_persistence_waits"] += 1
                            else:
                                diagnostic["breakout_waits"] += 1
                            continue
                        diagnostic["breakout_entries"] += 1

                    if mode == "MARKET":
                        diagnostic["edge_entries_market"] += 1
                    elif mode == "BREAKOUT":
                        diagnostic["edge_entries_breakout"] += 1
                    elif mode == "COMBO":
                        diagnostic["edge_entries_combo"] += 1
                ...
                if action == "OPEN_STRONG_BUY":
                    ...
                    if mode == "MARKET":
                        ...
                        prefix = "D3 BREAKOUT 50%"
                    elif mode == "COMBO":
                        prefix = "D3 MARKET+BREAKOUT 50%"
                    else:
                        prefix = "C기준 50%"
    ...
    return {
        ...
        "date": date_text,
        "strategy": f"D3_EDGE_ENTRY_{mode}",
        "mode": mode,
        ...
        "rules": {
            ...
            "edge_entry": {
                "opening_window": "frozen D-v2 unchanged",
                "ordinary_buy1_size": "original 50% unchanged when an entry is allowed",
                "MARKET": "block ordinary BUY1 only in RED regime; RED requires >=2 weak breadth/momentum flags",
                "BREAKOUT": "TOP3 leader for >=2 scans + VWAP>=0 + ret3/ret5>0 + close >= previous 5-bar high",
                "COMBO": "MARKET and BREAKOUT simultaneously",
                "exits": "STOP -3%, TAKE1 +3%, TAKE2 +5%, PROFIT_GUARD unchanged",
                "no_new_exit_rules": True,
                "no_adaptive_downsizing": True,
            },
        },
        "assumptions": {
            "real_orders": False,
            ...
            "fees_taxes": "별도 미포함",
            "purpose": "D3 market-regime + leader-persistence + breakout entry validation",
        },
    }

# =============================================================================
# 147-day D3 EDGE ENTRY orchestrator. Imported/called by replay_kr_long_backtest.
# =============================================================================
...
def _paths():
    base = _base()
    root = base.ROOT / "d3_edge_entry_full_engine"
    day_dir = root / "daily"
...
def _state(**updates) -> dict:
    ...
    with _LOCK:
        ...
        cur.update(updates)
        cur.setdefault("version", EDGE_VERSION)
...
def _public_state() -> dict:
    ...
    if not state:
        state = {"ok": True, "version": EDGE_VERSION, "status": "not_started", "result_ready": False}
    out = dict(state)
    out["ok"] = True
...
def _pack(x: dict) -> dict:
    ...
    realized = [int(e.get("실현손익KRW", 0) or 0) for e in events if str(e.get("구분", "")).upper() == "SELL"]
...
def _aggregate(label: str, key: str, rows: list[dict]) -> dict:
    ...
    diag_keys = [
        "market_red_scans", "market_blocked_signals", "leader_persistence_waits",
        "breakout_waits", "breakout_entries", "edge_entries_market",
        "edge_entries_breakout", "edge_entries_combo", "normal_stop_losses",
    ]
    diag = {
    ...
    return {
        ...
        "sell_event_win_rate_pct": round(100.0 * win_events / (win_events + loss_events), 2) if (win_events + loss_events) else 0.0,
...
def _profit_preservation(rows: list[dict], key: str) -> dict:
    ...
    variant_gp = int(sum(int((r.get(key) or {}).get("gross_profit_KRW", 0) or 0) for r in rows))
...
def _monthly(rows: list[dict], key: str) -> list[dict]:
    ...
    for r in rows:
        ...
        c = int((r.get("CONTROL_D2") or {}).get("pnl_KRW", 0) or 0)
        v = int((r.get(key) or {}).get("pnl_KRW", 0) or 0)
        m = out.setdefault(month, {"month": month, "days": 0, "control_KRW": 0, "variant_KRW": 0, "delta_KRW": 0})
        m["days"] += 1
        m["control_KRW"] += c
        m["variant_KRW"] += v
        m["delta_KRW"] += v - c
...
def _profitable_month_preservation(monthly_rows: list[dict]) -> dict:
    ...
    for m in monthly_rows:
        ...
        if c > 0:
            ...
            vals.append({"month": m.get("month"), "control_KRW": c, "variant_KRW": v, "preservation_pct": round(pct, 2)})
    return {
        "months": vals,
...
def _job(result: dict) -> None:
    ...
    try:
        ...
        _state(
            status="running", phase="D3_EDGE_ENTRY_FULL_ENGINE",
            started_at=datetime.now(KST).isoformat(timespec="seconds"),
            total_days=len(daily_base), completed_days=0, progress_pct=0.0,
            result_ready=False, error_days=0,
            message="CONTROL / MARKET / BREAKOUT / COMBO 진입전략 전체엔진 비교 시작",
            last_error="",
        )
        ...
        for idx, base_row in enumerate(daily_base, start=1):
            ...
            while True:
                ...
                if not live:
                    break
                _state(status="paused_live_window", phase="PAUSED", pause_reason=label,
                       current_date=date_text, completed_days=len(rows), total_days=len(daily_base),
                       message="실시간 자동매매 보호를 위해 D3 EDGE ENTRY 검증 일시정지")
                time.sleep(30.0)

            _state(status="running", phase="D3_EDGE_ENTRY_FULL_ENGINE", current_date=date_text,
                   completed_days=len(rows), error_days=len(errors), total_days=len(daily_base),
                   progress_pct=round(100.0 * (idx - 1) / len(daily_base), 1),
                   message=f"D3 EDGE ENTRY {idx}/{len(daily_base)} · {date_text}")

            day_path = _day_path(date_text)
            cached = base._load_gzip_json(day_path, None)
            if isinstance(cached, dict) and cached.get("version") == EDGE_VERSION:
                row = cached
            else:
                try:
                    control = run_kr_d3_edge_entry_replay(date_text=date_text, codes=codes, config=cfg, mode="CONTROL")
                    market = run_kr_d3_edge_entry_replay(date_text=date_text, codes=codes, config=cfg, mode="MARKET")
                    breakout = run_kr_d3_edge_entry_replay(date_text=date_text, codes=codes, config=cfg, mode="BREAKOUT")
                    combo = run_kr_d3_edge_entry_replay(date_text=date_text, codes=codes, config=cfg, mode="COMBO")
                    row = {
                        "version": EDGE_VERSION,
                        "date": date_text,
                        ...
                        "MARKET": _pack(market),
                        "BREAKOUT": _pack(breakout),
                        "COMBO": _pack(combo),
                    }
                    row["parity_delta_KRW"] = int(row["CONTROL_D2"]["pnl_KRW"] - row["cached_D2_KRW"])
                    for key in ("MARKET", "BREAKOUT", "COMBO"):
                        row[key]["delta_vs_control_KRW"] = int(row[key]["pnl_KRW"] - row["CONTROL_D2"]["pnl_KRW"])
            ...
            _state(status="running", phase="D3_EDGE_ENTRY_FULL_ENGINE", current_date=date_text,
                   ...
                   message=f"D3 EDGE ENTRY {idx}/{len(daily_base)} 완료")

        control = _aggregate("D-v2 원본 전체엔진", "CONTROL_D2", rows)
        market = _aggregate("MARKET · RED 장 일반 BUY1 차단", "MARKET", rows)
        breakout = _aggregate("BREAKOUT · 지속 TOP3 + 5-bar 돌파 확인", "BREAKOUT", rows)
        combo = _aggregate("COMBO · MARKET + BREAKOUT", "COMBO", rows)
        variants = [control, market, breakout, combo]
        for v in variants[1:]:
            v["delta_vs_control_KRW"] = int(v["total_KRW"] - control["total_KRW"])

        parity_ok = len(rows) == len(daily_base) and not errors and not parity_mismatches
        preservation = {k: _profit_preservation(rows, k) for k in ("MARKET", "BREAKOUT", "COMBO")}
        monthly = {"CONTROL_D2": _monthly(rows, "CONTROL_D2")}
        month_pres = {}
        for k in ("MARKET", "BREAKOUT", "COMBO"):
        ...
        for v in variants[1:]:
            ...
            if (
                ...
                and preservation[key]["passes_90pct_profit_preservation"]
                and month_pres[key]["passes_85pct_each_profitable_month"]
            ):
                accepted.append(v)
        best = max(accepted, key=lambda x: int(x["total_KRW"])) if accepted else None
        ...
        payload = {
            "ok": True,
            "version": EDGE_VERSION,
            "mode": "PATH_CONSISTENT_D3_EDGE_ENTRY_A_B_C_D",
            "read_only": True,
            ...
            "parity": {
                ...
                "rule": "CONTROL must match frozen cached D-v2 day-by-day before any D3 conclusion is accepted.",
            },
            "variants": variants,
            "profit_preservation": preservation,
            "profitable_month_preservation": month_pres,
            "monthly": monthly,
            "best_candidate_if_acceptance_passes": best,
            "acceptance": {
                "profit_preservation_min_pct": 90.0,
                "gross_profit_preservation_min_pct": 90.0,
                "each_original_profitable_month_min_pct": 85.0,
                "must_improve_total_KRW": True,
                "parity_required": True,
            },
            "top_market_improved_days": sorted([day_delta(r,"MARKET") for r in rows], key=lambda x:x["delta_KRW"], reverse=True)[:10],
            "top_market_worsened_days": sorted([day_delta(r,"MARKET") for r in rows], key=lambda x:x["delta_KRW"])[:10],
            "top_breakout_improved_days": sorted([day_delta(r,"BREAKOUT") for r in rows], key=lambda x:x["delta_KRW"], reverse=True)[:10],
            "top_breakout_worsened_days": sorted([day_delta(r,"BREAKOUT") for r in rows], key=lambda x:x["delta_KRW"])[:10],
            "top_combo_improved_days": sorted([day_delta(r,"COMBO") for r in rows], key=lambda x:x["delta_KRW"], reverse=True)[:10],
            "top_combo_worsened_days": sorted([day_delta(r,"COMBO") for r in rows], key=lambda x:x["delta_KRW"])[:10],
            "rules": {
                "CONTROL": "frozen D-v2",
                "MARKET": "opening D-v2 unchanged; after 09:20 ordinary BUY1 blocked only when broad regime is RED",
                "BREAKOUT": "opening D-v2 unchanged; ordinary BUY1 requires persistent TOP3 >=2 scans, VWAP>=0, ret3/ret5>0, previous five-bar high breakout",
                "COMBO": "MARKET + BREAKOUT",
                "exit_engine": "identical D-v2 STOP/TAKE1/TAKE2/PROFIT_GUARD in all modes",
                "position_size": "allowed ordinary BUY1 remains original 50%; no generic 25% downsizing",
            },
            "daily": rows,
            "important_limit": (
                ...
                "Fees/taxes remain excluded. This is a strategy-development comparison, not a promise of live performance."
            ),
        }
        _atomic_write_json(result_file, payload)
        _state(status="completed", phase="DONE", completed_days=len(rows), error_days=len(errors),
               total_days=len(daily_base), progress_pct=100.0, result_ready=True, parity_ok=parity_ok,
               finished_at=datetime.now(KST).isoformat(timespec="seconds"),
               message="D3 EDGE ENTRY MARKET/BREAKOUT/COMBO 전체엔진 검증 완료", last_error="")
    except Exception as exc:
        _state(status="error", phase="ERROR", result_ready=_paths()[3].exists(),
               last_error=f"{type(exc).__name__}: {exc}"[:1200], message="D3 EDGE ENTRY 검증 오류")


def ensure_d3_edge_entry_started(result: dict) -> dict:
    global _THREAD
    _, _, _, result_file = _paths()
    ...
    if (
        ...
        and existing.get("version") == EDGE_VERSION
    ...
    with _LOCK:
        ...
        _THREAD = threading.Thread(target=_job, args=(dict(result),), daemon=True, name="kr-d3-edge-entry-full-engine")
        _THREAD.start()
        state = _public_state()
        state["started"] = True
        return state
