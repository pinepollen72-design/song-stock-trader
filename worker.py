...
def main() -> int:
    ...
    # KIS 토큰은 여기서 한 번 준비합니다.
    # EGW00133(1분당 1회 제한)이면 KISClient 내부에서 65초 기다렸다 재시도하므로
    # worker가 몇 초마다 죽고 재시작되는 폭주를 막습니다.
    try:
        client.get_token()
        log("✅ KIS 접근토큰 준비 완료")
    except Exception as e:
        ...
        # 즉시 종료하면 Render가 몇 초 뒤 재시작해서 다시 토큰을 요청하므로
        # 충분히 기다린 뒤 종료합니다.
        log("⏳ 토큰 오류 재시작 폭주 방지를 위해 70초 대기 후 종료합니다.")
        time.sleep(70)
        return 3
