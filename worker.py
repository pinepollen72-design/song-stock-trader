...
CFG = AutoConfig(
    ...
    force_exit_all_demo_holdings=os.getenv(
        "FORCE_EXIT_ALL_DEMO_HOLDINGS", "true"
...
def _journal_append(journal: list, result: dict | None, market: str) -> list:
    ...
    for a in result.get("actions", []) or []:
        ...
        key = (
            f"{market}|{ts}|{a.get('symbol')}|{action}|"
            f"{a.get('qty')}|{order_status}"
        )
        if any(
...
def _kr_balance_snapshot(client: KISClient) -> dict:
    ...
    if rt_cd and rt_cd != "0":
        return {
            "ok": False,
            "at": at,
            "error": f"{raw.get('msg_cd', '')} {raw.get('msg1', '')}".strip(),
        }
...
def _us_balance_snapshot(client: KISClient) -> dict:
    ...
    for exchange in ("NASD", "NYSE", "AMEX"):
        ...
        if rt_cd and rt_cd != "0":
            errors.append(
                f"{exchange}: {raw.get('msg_cd', '')} {raw.get('msg1', '')}".strip()
            )
            continue

        success_calls += 1
        for r in (raw.get("output1", []) or []):
            symbol = str(
                r.get("ovrs_pdno")
                or r.get("pdno")
            ...
            qty = int(_num(r, "ovrs_cblc_qty", "hldg_qty", "hold_qty"))
            if qty <= 0:
                continue
            ...
            merged[symbol] = {
                ...
                "보유수량": qty,
                "평균매입가": _num(
                    r, "pchs_avg_pric", "avg_pric", "pchs_avg_pric2"
                ),
                "현재가": _num(
                    r, "now_pric2", "ovrs_now_pric1", "last", "prpr"
                ),
                "거래소": exchange,
            }
...
def _apply_balance_sync(status_updates: dict, prefix: str, snap: dict) -> None:
    status_updates[f"{prefix}_holdings_sync"] = {
        k: v for k, v in snap.items() if k != "holdings"
    }

    # 조회 성공일 때만 holdings를 덮어씁니다.
    # 조회 실패를 "보유잔고 없음"으로 오해하지 않도록 이전 성공값을 보존합니다.
    if snap.get("ok"):
...
def main() -> int:
    ...
    log(
        f"V2.1 Worker 시작 env={ENV}, "
        f"주문={'ON' if EXECUTE else 'DRY'}, "
        f"계좌끝4자리={client.account_no[-4:]}"
    )

    # 배포 직후 장이 닫혀 있어도 한국투자 실제 잔고를 즉시 읽습니다.
    initial_updates = {
        "running": True,
        "status": "running",
    ...
    _apply_balance_sync(
        initial_updates, "kr", _kr_balance_snapshot(client)
    )
    _apply_balance_sync(
        initial_updates, "us", _us_balance_snapshot(client)
    )
    ...
    while RUNNING:
        ...
        try:
            ...
            # 정규장 여부와 상관없이 한국투자 실제 잔고를 주기적으로 동기화합니다.
            if time.time() - last_balance_sync >= BALANCE_SYNC_SECONDS:
                _apply_balance_sync(
                    updates, "kr", _kr_balance_snapshot(client)
                )
                _apply_balance_sync(
                    updates, "us", _us_balance_snapshot(client)
                )
                last_balance_sync = time.time()
            ...
            if (
                ...
                if (
                    ...
                    try:
                        ...
                        log(
                            f"KR TOP5 오류: {type(e).__name__}: {e}"
                        )

                kr_result = run_kr_cycle(
                    client, kr_top5, CFG, EXECUTE
                )
                kr_journal = _journal_append(
                    kr_journal, kr_result, "KR"
                )

                updates["kr_last_result"] = _safe_result(kr_result)
                updates["kr_holdings"] = kr_result.get(
                    "holdings", []
                )
            ...
            if (
                ...
                if (
                    ...
                    try:
                        ...
                        log(
                            f"US TOP5 오류: {type(e).__name__}: {e}"
                        )

                us_result = run_us_cycle(
                    client, us_top5, CFG, EXECUTE
                )
                us_journal = _journal_append(
                    us_journal, us_result, "US"
                )

                updates["us_last_result"] = _safe_result(us_result)
                updates["us_holdings"] = us_result.get(
                    "holdings", []
                )
                updates["us_holdings_sync"] = {
                    "ok": True,
                    "at": us_result.get("time"),
                    "count": len(
                        us_result.get("holdings", []) or []
                    ),
                    "source": "자동매매 사이클 KIS 잔고조회",
                }
