...
US_UNIVERSE = [
    ...
    for x in os.getenv(
        "US_UNIVERSE",
        "AAPL,MSFT,NVDA,AMZN,META,TSLA,AVGO,GOOGL,AMD,NFLX,PLTR,ORCL,CRM,ADBE,INTC,QCOM,MU,AMAT,ARM,SMCI,JPM,BAC,GS,V,MA,COIN,HOOD,UBER,ABNB,WMT,COST,HD,NKE,DIS,BA,CAT,GE,LLY,UNH,XOM"
    ).split(",")
    if x.strip()
]
...
def _sync_us_budget_from_krw_local(config: AutoConfig):
...
def main() -> int:
    ...
    log(
        "🚀 완전자동 worker 시작 준비 "
        f"(env={WORKER_ENV}, 주문전송={'ON' if real_execute else 'DRY'})"
    )
    ...
    us_budget = _sync_us_budget_from_krw_local(cfg)
    log(
    ...
    log(
        "🇺🇸 미국장 한국시간: "
        f"후보준비 {_us_time_kst_label('09:00')} / "
        f"주문시작 {_us_time_kst_label('09:30')} / "
        f"신규매수 종료 {_us_time_kst_label('15:30')} / "
        f"강제청산 {_us_time_kst_label('15:50')} / "
        f"정규장 종료 {_us_time_kst_label('16:00')}"
    )
    log(
        "🤖 완전자동 모드: 버튼 필요 없음 → "
        "후보 자동갱신 → 조건충족 자동매수 → "
    ...
    while RUNNING:
        ...
        if time.time() - last_heartbeat >= 300:
            log(
                "💓 worker 정상 동작중 · "
                f"KR={'감시중' if in_kr_monitor_window(kr_now) else '대기'} · "
                f"US={'감시중' if in_us_monitor_window(us_now) else '대기'}"
            )
