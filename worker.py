...
from trader_core import Settings, KISClient, parse_domestic_holdings
...
REAL_CONFIRM = (
    os.getenv("REAL_WORKER_CONFIRM", "")
    == "I-UNDERSTAND-LIVE-ORDERS"
)

# 매우 중요:
# 실제 주문을 보내는 Worker는 반드시 하나만 둡니다.
# Railway를 PRIMARY로 쓰는 현재 구조에서는 Railway에만
# WORKER_PRIMARY=true를 넣고 Render는 false/미설정으로 둡니다.
PRIMARY = os.getenv(
    "WORKER_PRIMARY",
    "false",
).lower() in ("1", "true", "yes", "on")
...
LOOP_SECONDS = max(
    30,
    int(os.getenv("WORKER_LOOP_SECONDS", "60")),
)

KR_RESCAN_SECONDS = max(
    120,
    int(os.getenv("KR_RESCAN_SECONDS", "300")),
)

US_RESCAN_SECONDS = max(
    120,
    int(os.getenv("US_RESCAN_SECONDS", "300")),
)

BALANCE_SYNC_SECONDS = max(
    60,
    int(os.getenv("BALANCE_SYNC_SECONDS", "120")),
)
...
CFG = AutoConfig(
    kr_daily_budget=int(
        os.getenv("KR_DAILY_BUDGET", "10000000")
    ),
    kr_per_stock_budget=int(
        os.getenv("KR_PER_STOCK_BUDGET", "3000000")
    ),
    us_daily_budget_usd=float(
        os.getenv("US_DAILY_BUDGET", "5000")
    ),
    us_per_stock_budget_usd=float(
        os.getenv("US_PER_STOCK_BUDGET", "1500")
    ),
    max_positions=int(
        os.getenv("MAX_POSITIONS", "3")
    ),
    min_score=float(
        os.getenv("MIN_COMBINED_SCORE", "50")
    ),
    stop_loss_pct=float(
        os.getenv("STOP_LOSS_PCT", "3.0")
    ),
    take1_pct=float(
        os.getenv("TAKE1_PCT", "3.0")
    ),
    take2_pct=float(
        os.getenv("TAKE2_PCT", "5.0")
    ),
    kr_last_entry_time=os.getenv(
        "KR_LAST_ENTRY_TIME",
        "14:50",
    ),
    kr_force_exit_time=os.getenv(
        "KR_FORCE_EXIT_TIME",
        "15:15",
    ),
    us_last_entry_time=os.getenv(
        "US_LAST_ENTRY_TIME",
        "15:30",
    ),
    us_force_exit_time=os.getenv(
        "US_FORCE_EXIT_TIME",
        "15:50",
    ),
    buying_power_buffer_pct=float(
        os.getenv("BUYING_POWER_BUFFER_PCT", "5")
    ),
    confirm_wait_seconds=int(
        os.getenv("ORDER_CONFIRM_WAIT_SECONDS", "8")
    ),
...
def log(text: str) -> None:
    line = (
        f"[{datetime.now(KST).isoformat(timespec='seconds')}] "
        f"{text}"
    )
...
def _stage() -> tuple[str, str]:
    ...
    return (
        "WAIT",
        "⏳ 정규장 대기 중 · 한국투자 잔고는 계속 동기화",
    )


def _kr_balance_snapshot(
    client: KISClient,
) -> dict:
...
def _apply_balance_sync(
    status_updates: dict,
    prefix: str,
    snap: dict,
) -> None:
    status_updates[
        f"{prefix}_holdings_sync"
    ] = {
        k: v
    ...
    while RUNNING:
        ...
        try:
            ...
            if (
                time.time()
                - last_balance_sync
                >= BALANCE_SYNC_SECONDS
            ):
                _apply_balance_sync(
                    updates,
                    "kr",
                    _kr_balance_snapshot(
                        client
                    ),
                )
            ...
            updates.update(
                {
                    ...
                    "config": {
                        ...
                        "kr_force_exit_time": (
                            CFG.kr_force_exit_time
                        ),
                        "us_force_exit_time": (
                            CFG.us_force_exit_time
                        ),
                    },
                }
            )
