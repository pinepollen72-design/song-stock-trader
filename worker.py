...
OPEN_DEFENSE_IMPORT_ERROR = ""
try:
    from replay_kr_open_defense import run_kr_open_defense_replay
except Exception as _open_defense_import_e:
    OPEN_DEFENSE_IMPORT_ERROR = (
        f"{type(_open_defense_import_e).__name__}: {_open_defense_import_e}"
    )

    def run_kr_open_defense_replay(*args, **kwargs):
        raise RuntimeError(
            "OPEN DEFENSE replay import error: " + OPEN_DEFENSE_IMPORT_ERROR
        )

OPEN_DEFENSE_V2_IMPORT_ERROR = ""
try:
    from replay_kr_open_defense_v2 import run_kr_open_defense_replay as run_kr_open_defense_replay_v2
except Exception as _open_defense_v2_import_e:
    OPEN_DEFENSE_V2_IMPORT_ERROR = (
        f"{type(_open_defense_v2_import_e).__name__}: {_open_defense_v2_import_e}"
    )

    def run_kr_open_defense_replay_v2(*args, **kwargs):
        raise RuntimeError(
            "OPEN DEFENSE V2 replay import error: " + OPEN_DEFENSE_V2_IMPORT_ERROR
        )

D2_LONG_BACKTEST_IMPORT_ERROR = ""
try:
    from replay_kr_long_backtest import (
        start_d2_long_backtest,
        d2_long_backtest_status,
        d2_long_backtest_result,
        stop_d2_long_backtest,
    )
except Exception as _d2_long_import_e:
    D2_LONG_BACKTEST_IMPORT_ERROR = (
        f"{type(_d2_long_import_e).__name__}: {_d2_long_import_e}"
    )

    def start_d2_long_backtest(*args, **kwargs):
        return {
            "ok": False,
            "error": "D2 long backtest import error",
            "detail": D2_LONG_BACKTEST_IMPORT_ERROR,
        }

    def d2_long_backtest_status(*args, **kwargs):
        return {
            "ok": False,
            "error": "D2 long backtest import error",
            "detail": D2_LONG_BACKTEST_IMPORT_ERROR,
    ...
    def d2_long_backtest_result(*args, **kwargs):
        return {
            "ok": False,
            "error": "D2 long backtest import error",
            "detail": D2_LONG_BACKTEST_IMPORT_ERROR,
    ...
    def stop_d2_long_backtest(*args, **kwargs):
        return {
            "ok": False,
            "error": "D2 long backtest import error",
            "detail": D2_LONG_BACKTEST_IMPORT_ERROR,
...
def save_status(**updates) -> None:
    ...
    status.update({
        ...
        "provider": PROVIDER,
        "version": "2.11.2d-kis-tokenfix-ai-v2-d2-long-backtest",
        "status_api": "/status",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in (
            "/backtest-kr-d2-start",
            "/backtest-kr-d2-status",
            "/backtest-kr-d2-result",
            "/backtest-kr-d2-stop",
        ):
            values = query.get("view", [])
            supplied = str(values[0] or "").strip() if values else ""
            ...
            try:
                if path == "/backtest-kr-d2-start":
                    start_values = query.get("start", [])
                    end_values = query.get("end", [])
                    restart_values = query.get("restart", [])
                    start_date = (
                        str(start_values[0] or "").strip()
                        if start_values else "2026-01-01"
                    )
                    end_date = (
                        str(end_values[0] or "").strip()
                        if end_values else "2026-08-09"
                    )
                    restart = bool(
                        restart_values
                        and str(restart_values[0] or "").strip().lower()
                        in ("1", "true", "yes", "on")
                    )
                    payload = start_d2_long_backtest(
                        start_date=start_date,
                        end_date=end_date,
                        restart=restart,
                    )
                    code = 202 if payload.get("started") else 200
                elif path == "/backtest-kr-d2-status":
                    payload = d2_long_backtest_status()
                    code = 200
                elif path == "/backtest-kr-d2-stop":
                    payload = stop_d2_long_backtest()
                    code = 200
                else:
                    detail_values = query.get("detail", [])
                    detail = bool(
                        detail_values
                        and str(detail_values[0] or "").strip().lower()
                        in ("1", "true", "yes", "on")
                    )
                    payload = d2_long_backtest_result(detail=detail)
                    code = 200 if payload.get("ok") else 202
                ...
                body = json.dumps(
                    {
                        "ok": False,
                        "error": f"{type(e).__name__}: {e}",
                        "import_error": D2_LONG_BACKTEST_IMPORT_ERROR,
                    },
                    ensure_ascii=False,
                ).encode("utf-8")
                self._send_bytes(500, body, "application/json; charset=utf-8")
        ...
        if path == "/replay-kr-open-defense-v2":
            values = query.get("view", [])
            supplied = str(values[0] or "").strip() if values else ""
                ...
                body = json.dumps(
                    {
                        ...
                        "message": (
                            ...
                            "운영시간에는 OPEN DEFENSE V2 리플레이를 실행하지 않습니다."
                        ),
                    },
                    ensure_ascii=False,
                ).encode("utf-8")
            ...
            try:
                ...
                try:
                    replay = run_kr_open_defense_replay_v2(
                        date_text=date_text,
                        codes=codes,
                        refresh=refresh,
                    )
                    ...
                    body = json.dumps(
                        {
                            "ok": False,
                            "error": f"{type(e).__name__}: {e}",
                            "import_error": OPEN_DEFENSE_V2_IMPORT_ERROR,
                        },
                        ensure_ascii=False,
                    ).encode("utf-8")
                    self._send_bytes(500, body, "application/json; charset=utf-8")
        ...
        if path == "/replay-kr-open-defense":
            values = query.get("view", [])
            supplied = str(values[0] or "").strip() if values else ""
                ...
                body = json.dumps(
                    {
                        ...
                        "message": (
                            "실시간 자동매매 Worker 보호를 위해 한국장/미국장 "
                            "운영시간에는 OPEN DEFENSE 리플레이를 실행하지 않습니다."
                        ),
                    },
                    ensure_ascii=False,
                ).encode("utf-8")
            ...
            try:
                ...
                try:
                    replay = run_kr_open_defense_replay(
                        date_text=date_text,
                        codes=codes,
                        refresh=refresh,
                    )
                    ...
                    body = json.dumps(
                        {
                            "ok": False,
                            "error": f"{type(e).__name__}: {e}",
                            "import_error": OPEN_DEFENSE_IMPORT_ERROR,
                        },
                        ensure_ascii=False,
                    ).encode("utf-8")
                    self._send_bytes(500, body, "application/json; charset=utf-8")
