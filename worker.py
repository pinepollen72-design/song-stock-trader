...
def save_status(**kwargs) -> None:
    # 기존 상태를 유지하면서 전달받은 값만 갱신합니다.
    # 이렇게 해야 heartbeat 갱신 순간에도 TOP5/최근 결과/일지가 사라지지 않습니다.
    try:
...
def _journal_from_result(result, market: str, leaders: pd.DataFrame, previous):
    journal = list(previous or [])
    ...
    if isinstance(leaders, pd.DataFrame) and not leaders.empty:
        for _, row in leaders.iterrows():
            code = str(row.get("종목코드", row.get("종목", "")))
            name_map[code] = str(row.get("종목명", row.get("종목", code)))

    now_text = datetime.now(KST if market == "KR" else ET).isoformat(timespec="seconds")
    for item in result.get("actions", []) or []:
        if not isinstance(item, dict) or item.get("status") != "ORDERED":
            continue
        symbol = str(item.get("symbol", ""))
        action = str(item.get("action", ""))
        side = "매수" if action.startswith("BUY") else "매도"
        key = f"{market}|{symbol}|{action}|{item.get('qty')}|{now_text[:16]}"
        if any(str(x.get("_key", "")) == key for x in journal if isinstance(x, dict)):
            continue
        journal.append({
            "_key": key,
            "시간": now_text,
            "시장": "국내" if market == "KR" else "미국",
            "종목": name_map.get(symbol, symbol),
            "종목코드": symbol,
            "구분": side,
            "수량": int(item.get("qty", 0) or 0),
            "기준가": item.get("current_price", item.get("price", "")),
            "종합점수": item.get("combined_score", ""),
            "손익률": item.get("pnl", ""),
            "이유": str(item.get("reason", "")),
            "상태": "주문접수",
        })
    return journal[-200:]
...
def _safe_public_status() -> dict:
    ...
    return {
        ...
        "kr_journal": raw.get("kr_journal", []) if isinstance(raw.get("kr_journal", []), list) else [],
        "us_journal": raw.get("us_journal", []) if isinstance(raw.get("us_journal", []), list) else [],
    }
...
def make_config() -> AutoConfig:
    ...
    cfg.leader_exception_enabled = False
...
def main() -> int:
    ...
    try:
        _old = json.loads(WORKER_STATUS.read_text(encoding="utf-8")) if WORKER_STATUS.exists() else {}
    except Exception:
        _old = {}
    kr_journal = list(_old.get("kr_journal", []) or [])
    us_journal = list(_old.get("us_journal", []) or [])
    kr_last_scan = 0.0
    us_last_scan = 0.0
        ...
        try:
            if in_kr_monitor_window(kr_now):
                ...
                try:
                    ...
                    kr_journal = _journal_from_result(kr_result, "KR", kr_leaders, kr_journal)
                    if actions:
            ...
            if in_us_monitor_window(us_now):
                ...
                try:
                    ...
                    us_journal = _journal_from_result(us_result, "US", us_leaders, us_journal)
                    if actions:
            ...
            if kr_result is not None:
                status_extra["kr_last_result"] = kr_result
            if us_result is not None:
                status_extra["us_last_result"] = us_result
            save_status(**status_extra)
