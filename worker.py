...
def _resolve_state_dir() -> Path:
    """
    자동매매 상태/일지를 가능한 한 영구 저장소에 보관합니다.

    우선순위:
    1) SONG_TRADER_STATE_DIR 직접 지정
    2) Railway Volume이 마운트되어 있으면 RAILWAY_VOLUME_MOUNT_PATH
    3) 마지막 fallback은 /tmp/song_trader_v2 (재배포/재시작 시 유실 가능)
    """
...
def _merge_journals(*sources: list) -> list:
    """
    여러 저장원(status + 별도 journal 파일)을 _key 기준으로 합칩니다.
    과거 Worker가 _key 없이 저장한 행도 내용 기반으로 중복을 막습니다.
    """
    merged = []
    seen = set()
    ...
    status.update(
        {
            ...
            "provider": PROVIDER,
            "version": "2.4-persistent-journal",
            "status_api": "/status",
...
def main() -> int:
    ...
    kr_journal = _merge_journals(
        old.get("kr_journal", []) or [],
        journal_store.get("kr_journal", []) or [],
    )
    us_journal = _merge_journals(
        old.get("us_journal", []) or [],
        journal_store.get("us_journal", []) or [],
    )
    ...
    if not STATE_IS_PERSISTENT:
        log(
            "⚠️ 상태 저장소가 /tmp 입니다. Railway 재배포/재시작 시 일지가 "
            "사라질 수 있습니다. Railway Volume을 연결하거나 "
            "SONG_TRADER_STATE_DIR을 영구 볼륨 경로로 지정하세요."
        )
    else:
