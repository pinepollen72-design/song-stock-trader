...
def _download_intraday(date_text: str, universe) -> tuple[dict[str, pd.DataFrame], dict[str, dict]]:
    ...
    for code, name, exch in universe:
        ...
        for suffix in suffixes:
            ...
            ticker_meta[ticker] = {"code": code, "name": name, "exchange": suffix}
    ...
    for code, candidates in ticker_candidates.items():
        ...
        for ticker in candidates:
            ...
            if not d.empty:
                target_day = d[d.index.strftime("%Y-%m-%d") == date_text]
...
def _bars_until(frame: pd.DataFrame, cutoff: pd.Timestamp, date_text: str) -> pd.DataFrame:
    # 1분봉의 close는 그 1분이 끝난 뒤 알 수 있으므로 60초 전까지만 사용한다.
    safe_cutoff = cutoff - pd.Timedelta(seconds=60)
    d = frame[
        (frame.index.strftime("%Y-%m-%d") == date_text)
        & (frame.index <= safe_cutoff)
    ].copy()
    return d
...
def _candidate_base_at(frames, meta, date_text: str, cutoff: pd.Timestamp) -> pd.DataFrame:
    ...
    for code, frame in frames.items():
        ...
        volume = float(pd.to_numeric(d.get("Volume", 0), errors="coerce").fillna(0).sum())
        amount = float((pd.to_numeric(d["Close"], errors="coerce").fillna(0) * pd.to_numeric(d.get("Volume", 0), errors="coerce").fillna(0)).sum())
    ...
    return out.sort_values(["주도주점수", "거래대금", "누적거래량"], ascending=False).reset_index(drop=True)
...
def _price_at(frame: pd.DataFrame, date_text: str, now: pd.Timestamp) -> float:
    d = _bars_until(frame, now, date_text)
    if d.empty:
        return 0.0
    try:
        ...
        for symbol, pos in list(positions.items()):
            ...
            if (
                ...
                momentum_ok = bool(
                    ...
                    and float(row.get("종합점수", 0) or 0) >= cfg.min_score
                )
                if momentum_ok:
                    ...
                    if qty2 > 0 and daily_buy_amount + cost <= cfg.daily_budget_krw:
                        ...
                        add_event(now, symbol, "BUY2", "BUY", qty2, ref_price, fill,
                                  f"2차 분할매수 +{pnl:.2f}% · 최신 모멘텀 유지", pnl, 0.0, score, rank)
                        pos["qty"] = new_qty
