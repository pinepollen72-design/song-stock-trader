...
class AutoConfig:
    ...
    # 모의투자 완화 기준
    # demo에서는 종합점수 55 이상이면 녹색 신호가 아니어도
    # TOP5 후보에 한해 1차 진입을 허용합니다.
    demo_relaxed_entry_enabled: bool = True
    demo_min_combined_score: float = 55.0
...
def run_domestic_cycle(
    ...
    for symbol, pos in list(state["positions"].items()):
        ...
        # 대장주 예외 또는 demo 완화진입은
        # 첫 매수만 허용하고 2/3차 추매는 막습니다.
    ...
    for _, row in leader_df.iterrows():
        ...
        # 모의투자 전용 완화 기준
        # demo에서만 적용
        # TOP5 + 종합점수 55 이상이면 녹색 신호 없어도 1차 진입
        # -------------------------------------------------
        demo_relaxed_entry = (
            is_demo
            and bool(getattr(config, "demo_relaxed_entry_enabled", True))
            and combined >= float(
                getattr(config, "demo_min_combined_score", 55.0)
            )
        )
        ...
        # 우선순위: 정상 > 대장주 예외 > demo 완화
        if normal_entry:
            entry_reason = (
                f"정상 1차 분할매수 · 종합점수 {combined:.1f} · {signal}"
            )
            action_name = "BUY1"
            relaxed_flag = False
            ...
            entry_reason = (
                f"모의완화 1차매수 · TOP5 · 종합점수 {combined:.1f} · {signal}"
            )
    ...
    if not result["actions"]:
        if is_demo:
            result["message"] = (
                ...
                f"(demo 기준 종합점수 {getattr(config, 'demo_min_combined_score', 55.0):.0f}+)"
            )
        else:
...
def run_overseas_cycle(
    ...
    for symbol, pos in list(state["positions"].items()):
        ...
        qty_for_manage = (
            actual_qty
            if actual_qty > 0
            else int(pos.get("expected_qty", 0))
        )
