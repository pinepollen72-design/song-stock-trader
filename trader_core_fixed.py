...
TOKEN_DIR = Path(os.getenv("SONG_TRADER_STATE_DIR", "/tmp/song_trader_v2"))
...
class KISClient:
    def __init__(self, settings: Settings, env: str = "demo"):
        ...
        self._rest_min_interval = max(
            0.0,
            float(os.getenv("KIS_REST_MIN_INTERVAL", default_interval)),
        )
    ...
    def overseas_present_balance_us(
        self,
        foreign_currency: bool = True,
    ) -> Dict[str, Any]:
        """해외주식 체결기준현재잔고. 모의 TR: VTRP6504R"""
    ...
    def overseas_all_us_balances(self) -> List[Dict[str, Any]]:
        """
        미국 3개 거래소의 결제기준 잔고를 모두 조회한다.
        NVDA처럼 NASDAQ 종목뿐 아니라 NYSE/AMEX 보유종목도 함께 동기화하기 위한 용도.
        """
...
def parse_overseas_holdings(
    ...
    """
    KIS 해외주식 잔고 응답을 공통 형식으로 변환한다.

    - inquire-balance(VTTS3012R/TTTS3012R)의 output1
    - inquire-present-balance(VTRP6504R/CTRP6504R)의 output1
      등에서 필드명이 조금 달라도 최대한 자동 인식한다.
    """
    empty = pd.DataFrame(
        columns=[
            "종목코드", "종목명", "거래소", "보유수량",
    ...
    # 일부 응답/래퍼에서 output이 바로 리스트인 경우도 허용
    ...
    code = first_existing(
        "ovrs_pdno", "pdno", "symb", "item_cd", "ovrs_item_cd"
    )
    name = first_existing(
        "ovrs_item_name", "prdt_name", "item_name", "ovrs_prdt_name"
    )
    exch = first_existing(
        "ovrs_excg_cd", "ovrs_excg_name", "tr_mket_name", "excg_cd"
    )
    qty = first_existing(
        "ovrs_cblc_qty", "cblc_qty13", "hldg_qty", "hold_qty",
        "ord_psbl_qty", "ovrs_stck_qty"
    )
    sellable = first_existing(
        "ord_psbl_qty", "sell_psbl_qty", "sll_psbl_qty", "ovrs_ord_psbl_qty"
    )
    avg = first_existing(
        "pchs_avg_pric", "avg_unpr3", "pchs_avg_pric2",
        "frcr_pchs_avg_pric", "avg_pric"
    )
    cur = first_existing(
        "now_pric2", "ovrs_now_pric1", "ovrs_now_pric",
        "last", "prpr"
    )
    eval_amt = first_existing(
        "ovrs_stck_evlu_amt", "frcr_evlu_amt2", "evlu_amt",
        "ovrs_evlu_amt"
    )
    pnl = first_existing(
        "frcr_evlu_pfls_amt", "evlu_pfls_amt2", "evlu_pfls_amt",
        "ovrs_evlu_pfls_amt"
    )
    pnl_rate = first_existing(
        "evlu_pfls_rt", "evlu_pfls_rt1", "pfls_rt", "profit_loss_rate"
    )

    out = pd.DataFrame(index=df.index)
    out["종목코드"] = (
        df[code].astype(str).str.strip().str.upper() if code else ""
    )
    out["종목명"] = df[name].astype(str).str.strip() if name else ""

    fallback_exchange = (
        str(balance_json.get("_exchange") or default_exchange or "")
        .strip()
        .upper()
    )
    out["거래소"] = (
        df[exch].astype(str).str.strip().str.upper()
        if exch else fallback_exchange
    )
    ...
    # 평가금액/손익이 응답에 없으면 계산 가능한 범위에서 보완
    missing_eval = out["평가금액"].eq(0) & out["현재가"].gt(0) & out["보유수량"].gt(0)
    out.loc[missing_eval, "평가금액"] = (
        out.loc[missing_eval, "현재가"] * out.loc[missing_eval, "보유수량"]
    )
    ...
    out.loc[missing_rate, "수익률"] = (
        (out.loc[missing_rate, "현재가"] / out.loc[missing_rate, "평균매입가"] - 1.0)
        * 100.0
    )

    # KIS가 수량을 소수 문자열로 줄 수 있어 float 유지.
    # 실제 미국주식 모의 일반주문은 보통 정수지만 파싱 단계에서 값 손실을 막는다.
    out = out[out["보유수량"] > 0].copy()

    # 종목코드가 비어 있는 비정상 행 제거
    out = out[out["종목코드"].astype(str).str.len() > 0]

    return out.reset_index(drop=True)


def merge_overseas_holdings(
    balance_responses: List[Dict[str, Any]],
) -> pd.DataFrame:
    """
    NASD/NYSE/AMEX 등 여러 잔고 응답을 하나로 합친다.
    같은 종목이 중복으로 내려오는 경우 종목코드+거래소 기준으로 1건만 남긴다.
    """
