...
def _v2_timeout_seconds() -> float:
    """V2 리플레이 전용 OpenAI 타임아웃.

    실시간 shadow 위원회보다 입력이 크므로 기본 8초를 그대로 쓰지 않는다.
    Railway Variables에서 AI_COMMITTEE_V2_REPLAY_TIMEOUT_SECONDS로 조정 가능하다.
    """
    raw = os.getenv("AI_COMMITTEE_V2_REPLAY_TIMEOUT_SECONDS", "30").strip()
    try:
        return max(10.0, min(90.0, float(raw)))
    except Exception:
        return 30.0


def _v2_max_attempts() -> int:
    raw = os.getenv("AI_COMMITTEE_V2_REPLAY_MAX_ATTEMPTS", "2").strip()
    try:
...
def _call_openai_v2(
    ...
    timeout_seconds = _v2_timeout_seconds()
    max_attempts = _v2_max_attempts()
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            if not response.ok:
                ...
                # 429/5xx는 일시 오류일 수 있어 재시도하고, 4xx는 즉시 원인을 노출한다.
            ...
            last_error = RuntimeError(
                f"OpenAI V2 timeout after {timeout_seconds:.0f}s "
            ...
            last_error = RuntimeError(
                f"OpenAI V2 network error {type(e).__name__}: {str(e)[:300]}"
            )
            if attempt < max_attempts:
                time.sleep(1.5 * attempt)
                continue
    ...
    raise last_error or RuntimeError("OpenAI V2 호출 실패")
...
def _simulate_ai_v2_auditor(
            ...
            if eligible_rows:
                ...
                try:
                    ...
                    ai_error_details.append({
                        "time": now.isoformat(),
                        "stage": "batch",
                        "symbols": [str(x.get("종목코드", "")).zfill(6) for x in batch_rows],
    ...
    return {
        ...
        "api_calls": api_calls,
        "ai_errors": ai_errors,
        "ai_error_details": ai_error_details[-20:],
...
def run_kr_ai_committee_replay_v2(
    ...
    if not refresh and not codes and cache.exists():
        ...
        if (
            ...
            and int(cached.get("ai_errors", 0) or 0) == 0
            and bool(cached.get("ai_v2_valid", True))
        ):
            cached["cached"] = True
            return cached
        # 이전 FAIL_OPEN 결과는 정상 V2 결과가 아니므로 자동 폐기하고 다시 계산한다.
    ...
    ai_error_count = int(v2.get("ai_errors", 0) or 0)
    baseline_buy_orders = int((baseline.get("summary", {}) or {}).get("매수주문횟수", 0) or 0)
    decision_total = sum(counts.values())
    ai_v2_valid = bool(
        ai_error_count == 0
        and (decision_total > 0 or baseline_buy_orders == 0)
    )

    payload = {
        "ok": True,
        "version": AI_REPLAY_V2_VERSION,
        "ai_v2_valid": ai_v2_valid,
        "date": date_text,
        "model": AI_CFG.model,
        "policy": {
            "baseline": "KR_C_NO_BUY2",
        ...
        "ai_error_details": list(v2.get("ai_error_details", []) or []),
        "v2_diagnostic": {
            "valid": ai_v2_valid,
            "timeout_seconds": _v2_timeout_seconds(),
            "max_attempts": _v2_max_attempts(),
            ...
            "message": (
                "AI V2 정상 판단 완료"
                if ai_v2_valid
                else "AI V2 판단 오류가 있어 이 결과를 C/V1/V2 비교에 사용하면 안 됩니다."
            ),
        },
