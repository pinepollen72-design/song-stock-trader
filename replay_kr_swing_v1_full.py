...
"""
KR SWING V1 — Trend + Pullback + Re-acceleration
...
SWING_VERSION = "kr-swing-v1-trend-pullback-fast-v1"
...
ROOT = _resolve_state_dir() / "replays" / "kr_swing_v1"
RAW_DAILY_DIR = ROOT / "kis_daily_ohlcv"
...
class SwingConfig:
    ...
    neutral_min_rs_percentile: float = 0.80
    min_volume_ratio: float = 0.90
    neutral_min_volume_ratio: float = 1.05
    min_pullback_pct: float = 2.0
    max_pullback_pct: float = 10.0
    max_extension_from_sma20_pct: float = 8.0
    ...
    neutral_breadth: float = 0.40
    risk_on_min_median_ret20: float = -0.02
...
def _candidate_rows(
    ...
    min_rs = (
        ...
        if regime == "RISK_ON"
        else cfg.neutral_min_rs_percentile
    )
    min_vol = (
        cfg.min_volume_ratio
        if regime == "RISK_ON"
        else cfg.neutral_min_volume_ratio
    )
    ...
    for code, df in frames.items():
        ...
        required = [
            "SMA20", "SMA60", "SMA20_5AGO", "ATR14",
            "VOL_RATIO", "RET20", "HIGH20_PREV",
            "HIGH3_PREV", "LOW5_PREV", "PULLBACK_DEPTH",
            "EXT_SMA20",
        ]
        ...
        location_ok = (
            extension <= cfg.max_extension_from_sma20_pct / 100.0
            and close <= high20 * (
                1.0 + cfg.max_breakout_extension_pct / 100.0
            )
        )
        flow_ok = (
            rs_pct >= min_rs
        ...
        # 0~100-ish score, used only to rank simultaneous valid setups.
        score = (
            rs_pct * 45.0
            + min(20.0, max(0.0, ret20 * 100.0))
            + min(15.0, max(0.0, (vol_ratio - 0.8) * 12.0))
            + max(
                0.0,
                20.0 - abs(pullback * 100.0 - 5.0) * 3.0,
            )
        )
...
def _run_backtest(
    ...
    for i, day in enumerate(days):
        ...
        for sig in todays_pending:
            ...
            market_limit = (
                ...
                else cfg.neutral_max_positions
            )
            if (
    ...
    return {
        "ok": True,
        "version": SWING_VERSION,
        "strategy": "SWING_V1_TREND_PULLBACK_REACCEL",
        "period": {
        ...
        "rules": {
            ...
            "market": (
                "fixed-universe breadth: RISK_ON / NEUTRAL / RISK_OFF"
            ),
            "trend": "Close>SMA20>SMA60 and rising SMA20",
            "pullback": "recent pullback depth 2%~10%",
            "reacceleration": "close breaks previous 3-day high",
            "relative_strength": "top 30%; neutral requires top 20%",
            "entry": "max 3 positions; max 2 new/day",
            "size": (
        ...
        "assumptions": {
            ...
            "purpose": (
                "strategy-family research only; not production approval"
            ),
        },
        "config": asdict(cfg),
        "trades": trades,
