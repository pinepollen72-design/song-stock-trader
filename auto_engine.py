...
def _kr_signal_fresh(row, now: datetime, config: AutoConfig) -> bool:
    ...
    if not raw:
        return True  # 구버전 strategy_kr 호환
    try:
...
def run_domestic_cycle(
    ...
    for symbol, pos in list(state["positions"].items()):
        ...
        if actual_qty <= 0:
            # 매도 주문의 잔고반영이 확인되어 0주가 된 경우 즉시 청산완료 처리.
        ...
        # 고정 익절선 전이라도 수익이 났다가 빠르게 되밀리면 수익을 보호한다.
        if (
        ...
        # 국내 추가매수는 딱 1번만: BUY2. 가격 상승 + 최신 모멘텀 유지가 모두 필요.
...
def _us_signal_fresh(row, now: datetime, config: AutoConfig) -> bool:
    raw = str(row.get("스캔시각", "") or "").strip()
    if not raw:
        return True  # 이전 strategy_us와의 호환
    try:
        ts = datetime.fromisoformat(raw)
        if ts.tzinfo is None:
...
def _effective_us_sellable(actual_qty: int, raw_sellable: int) -> int:
    """
    매도가능수량 필드가 정상 제공되면 그 값을 우선한다.
    일부 모의 응답에서 필드가 0/누락되는 경우에는 pending 주문 잠금이 중복매도를 막으므로
    최초 주문에 한해 실제 보유수량을 안전한 fallback으로 사용한다.
    """
...
def run_overseas_cycle(
    ...
    # ---------------------------------------------------------------
    # 1) 보유 종목 관리: 잔고 -> pending 확인 -> 손절/익절/추가매수
    # ---------------------------------------------------------------
    for symbol, pos in list(state["positions"].items()):
        actual = holdings.get(symbol, {})
        ...
        # 주문 접수 후 KIS 잔고에 반영될 때까지 같은 종목 추가 주문 금지.
        pending = pos.get("pending_order")
        ...
        def place_sell(action: str, qty: int, reason: str):
        ...
        # 강제청산은 가장 우선: 남은 실제 보유를 정규장 안에서 정리.
        ...
        if pnl <= -abs(float(config.stop_loss_pct)):
            place_sell("STOP_LOSS", sellable_qty, f"손절 {pnl:.2f}%")
            continue

        # 5%를 바로 넘어도 1차를 건너뛰지 않는다.
        if pnl >= float(config.take1_pct) and not pos.get("take1_sent"):
            sell_qty = max(1, actual_qty // 2)
            act = place_sell("TAKE1", sell_qty, f"1차 익절 {pnl:.2f}% · 약 50%")
            if act and act.get("status") == "DRY":
                pos["take1_sent"] = True
            continue

        if pnl >= float(config.take2_pct) and pos.get("take1_sent"):
            place_sell("TAKE2", sellable_qty, f"2차 익절 {pnl:.2f}% · 잔여 전량")
            continue

        # 고정 +3/+5% 전에 수익이 났다가 빠르게 꺾이면 수익을 지킨다.
        if (
        ...
        # 2차 매수는 가격 상승만 보지 않고 최신 모멘텀 유지까지 확인.
        if stage == 1 and pnl >= float(config.us_add2_trigger_pct):
            leader_row = leader_rows.get(symbol)
            if bool(config.us_require_momentum_confirm) and not _us_momentum_ok(leader_row, now, config):
                _diag(
                    result,
                    symbol,
                    "SKIP_BUY2_MOMENTUM",
    ...
    # ---------------------------------------------------------------
    # 2) 신규 진입: 최신 1분봉 기반 TOP5 중 강한 종목만 1차 50%
    # ---------------------------------------------------------------
    for _, row in leader_df.head(max(1, int(config.us_new_entry_top_n))).iterrows():
        ...
        if not symbol or symbol in state["positions"] or symbol in holdings:
            continue
