...
US_ORDER_LOCK_FILE = STATE_DIR / "us_order_lock.json"
...
class AutoConfig:
    ...
    duplicate_guard_seconds: int = 180
    ...
    # 09:00 ET부터 후보 준비. 앱/worker에서는 현재 DST를 반영해 KST로 표시.
...
def reset_us_state() -> None:
    for p in (US_STATE_FILE, US_ORDER_LOCK_FILE):
        try:
            if p.exists():
                p.unlink()
        except Exception:
...
def _read_us_order_lock() -> Dict[str, Any]:
    if not US_ORDER_LOCK_FILE.exists():
        return {}
    try:
        return json.loads(
            US_ORDER_LOCK_FILE.read_text(encoding="utf-8")
        )
    except Exception:
        return {}


def _write_us_order_lock(
    symbol: str,
    side: str,
    source: str,
) -> None:
    try:
        US_ORDER_LOCK_FILE.write_text(
            json.dumps(
                {
...
def _us_duplicate_guard_hit(
    symbol: str,
    side: str,
    seconds: int = 180,
):
    lock = _read_us_order_lock()
    if not lock:
        return False, ""

    if (
        str(lock.get("symbol", "")).upper()
        != str(symbol).upper()
        or str(lock.get("side", "")).lower()
        != str(side).lower()
    ):
        return False, ""
...
def _place_overseas_order(
    ...
    hit, why = _us_duplicate_guard_hit(
        symbol,
        side,
        180,
    )
