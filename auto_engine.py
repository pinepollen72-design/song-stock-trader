...
class AutoConfig:
    ...
    # 국내 C 전략:
    # BUY1은 종목당 예산의 50%만 진입하고 BUY2는 사용하지 않는다.
    # 남은 자금은 같은 종목 추가매수 대신 다음 좋은 종목 기회에 사용한다.
    kr_buy2_enabled: bool = False
    ...
    # 국내 BUY1 기준 비중은 50%. BUY2 OFF 시 나머지 50%는 다음 기회를 위해 남긴다.
...
def run_domestic_cycle(
    ...
    for symbol, pos in list(state["positions"].items()):
        ...
        if (
            bool(config.kr_buy2_enabled)
            and stage == 1
            and pnl >= float(config.kr_add2_trigger_pct)
        ):
