...
def _safe_us_buy_qty_from_buying_power(
    ...
    # '외화' 주문가능금액을 우선. 없으면 통합/주문가능외화 필드로 보완합니다.
    candidates = [
        _to_float_us(output.get("ovrs_ord_psbl_amt", 0)),
        _to_float_us(output.get("ord_psbl_frcr_amt", 0)),
        _to_float_us(output.get("frcr_ord_psbl_amt1", 0)),
        _to_float_us(output.get("echm_af_ord_psbl_amt", 0)),
    ]
    available_usd = max(candidates) if candidates else 0.0

    qty_candidates = [
        _to_int_us(output.get("max_ord_psbl_qty", 0)),
        _to_int_us(output.get("ord_psbl_qty", 0)),
        _to_int_us(output.get("ovrs_max_ord_psbl_qty", 0)),
        _to_int_us(output.get("echm_af_ord_psbl_qty", 0)),
    ]
    available_qty = max(qty_candidates) if qty_candidates else 0
    ...
    if available_usd <= 0:
        return 0, meta, "미국 주문가능금액이 0달러입니다."
    ...
    reason = (
        f"미국 매수가능금액 확인: ${available_usd:.2f} → "
        f"5% 안전여유 후 ${buffered_usd:.2f} · {qty}주 (${cost:.2f})"
    )
    return qty, meta, reason
