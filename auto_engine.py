...
def _extract_available_usd(balance_json: Dict[str, Any]) -> float:
    """
    해외잔고 응답에서 '주문가능' 의미가 명확한 필드만 사용합니다.
    누적매수금액(frcr_buy_amt_smtl*)은 주문가능현금이 아니므로 절대 사용하지 않습니다.
    """
    containers = []
    ...
    for container in containers:
        for key in keys:
            try:
                value = float(str(container.get(key, 0) or 0).replace(",", ""))
                if value > 0:
                    return value
            except Exception:
                pass
...
def _query_us_buying_power(
    client,
    symbol: str,
    price: float,
    exchange: str = "NASD",
) -> tuple[float, int, Dict[str, Any], str]:
    """주문 직전 해외주식 매수가능금액/최대수량을 직접 조회합니다."""
    try:
        raw = client.overseas_buying_power(
            symbol=symbol,
            reference_price=float(price),
            exchange=exchange,
        )
    except Exception as e:
        return 0.0, 0, {}, f"미국 매수가능조회 실패: {type(e).__name__}: {e}"

    if not isinstance(raw, dict):
        return 0.0, 0, {}, "미국 매수가능조회 응답 형식 이상"

    if str(raw.get("rt_cd", "")) not in ("0", ""):
        return 0.0, 0, raw, (
            f"미국 매수가능조회 거절: {raw.get('msg_cd', '')} {raw.get('msg1', '')}"
        )
    ...
    def num(*keys):
        for key in keys:
            try:
                v = float(str(output.get(key, 0) or 0).replace(",", ""))
                if v > 0:
                    return v
            except Exception:
                pass
    ...
    return amount, qty, raw, ""


def _us_safe_qty_live(
    client,
    config: AutoConfig,
    symbol: str,
    stage: int,
    price: float,
    fallback_available_usd: float = 0.0,
    exchange: str = "NASD",
) -> tuple[int, float, str, Dict[str, Any]]:
    """
    1/2/3차 매수마다 실제 주문가능 USD를 다시 조회하고 5% 안전여유를 적용합니다.
    조회가 실패하면 잔고 응답의 명시적 주문가능금액만 fallback으로 사용합니다.
    """
    stage_budget = _us_stage_budget(config, stage)
    amount, max_qty, raw, err = _query_us_buying_power(
        client, symbol, price, exchange
    )

    source = "inquire-psamount"
    if amount <= 0 and fallback_available_usd > 0:
        amount = float(fallback_available_usd)
        max_qty = 0
        source = "balance-fallback"

    if amount <= 0:
        return 0, 0.0, (err or "미국 주문가능 USD를 확인하지 못해 안전상 매수 보류"), {
            "source": source,
            "raw": raw,
        }
    ...
    buffered = amount * (1.0 - buffer_ratio)
    ...
    meta = {
        "source": source,
        "available_usd": round(amount, 2),
        "buffer_pct": float(config.buying_power_buffer_pct),
        "buffered_available_usd": round(buffered, 2),
        "stage_budget_usd": round(stage_budget, 2),
        "usable_usd": round(usable, 2),
                    "buying_power": bp,
        "max_order_qty": int(max_qty),
    }

    if qty <= 0:
        return 0, usable, (
            f"미국 주문가능수량 0: 안전여유 후 ${usable:.2f} / 현재가 ${price:.2f}"
        ), meta

    why = (
        f"실제 주문가능 ${amount:.2f} → 5% 여유 후 ${buffered:.2f} · "
        f"이번 차수 ${usable:.2f} / {qty}주"
    )
    return qty, usable, why, meta
...
def run_overseas_cycle(
    ...
    for symbol, pos in list(
        ...
        if stage == 1 and pnl >= float(config.add2_trigger_pct):
            qty, usable, why, bp = _us_safe_qty_live(
                client, config, symbol, 2, price, available_usd
            )
            cost = qty * price
        ...
        if stage == 2 and pnl >= float(config.add3_trigger_pct):
            qty, usable, why, bp = _us_safe_qty_live(
                client, config, symbol, 3, price, available_usd
            )
            cost = qty * price
    ...
    for _, row in leader_df.head(5).iterrows():
        ...
        qty, usable, why, bp = _us_safe_qty_live(
            client, config, symbol, 1, price, available_usd
        )
