...
D3_HYBRID_VERSION = "kr-d3-v2-hybrid-full-engine-v2"
...
def _ordinary_risky(row: Any, vwap_gap: float) -> tuple[bool, str]:
    ...
    # This function is called only after the frozen D-v2 BUY-candidate gate.
    # Therefore risk rules must overlap with a valid BUY candidate.
    below_vwap_risk = bool(
        vwap_gap <= RULES.vwap_risk_cutoff_pct
        and (m["ret3"] <= 0.20 or m["ret5"] <= 0.30)
    )
    strong_overheat_exception = bool(
        m["ret3"] >= 1.50
        and m["ret5"] >= 1.50
        and m["vol"] >= 1.50
    )
    weak_overheat = bool(
        vwap_gap >= RULES.overheat_vwap_gap_pct
        and not strong_overheat_exception
    )
    ...
    if weak_overheat:
        return True, (
            ...
            f"3/5분 {m['ret3']:+.2f}/{m['ret5']:+.2f}% · 거래량 {m['vol']:.2f}배"
        )
    return False, ""
    ...
    try:
        ...
        payload = {
            ...
            "rules": {
                ...
                "ordinary_entry": "50% normally; 25% when VWAP<=-0.5% with only small-positive momentum (ret3<=0.20 or ret5<=0.30), or VWAP>=+3.0% unless ret3/ret5>=+1.5% and volume>=1.5x; recovered risk entry may confirm back to 50%",
