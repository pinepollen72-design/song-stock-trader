...
class ReplayTradeConfig:
    ...
    # 조기청산 후 재진입 제어 실험
    # NONE / BAN_SAME / BAN_SAME_PAUSE
    reentry_control_mode: str = "NONE"
    pause_after_early_exits_count: int = 2
    pause_new_entries_minutes: float = 60.0
...
def run_trade_replay(
    ...
    # 조기청산 후 재진입 제어 상태
    early_exit_banned_symbols: set[str] = set()
    early_exits_since_pause: int = 0
    new_entry_pause_until: pd.Timestamp | None = None
    ...
    while now <= end:
        ...
        for symbol in list(positions.keys()):
            ...
            if bool(early_state.get("trigger")):
                ...
                reentry_mode = str(
                    config.reentry_control_mode or "NONE"
                ).upper()

                if reentry_mode in {"BAN_SAME", "BAN_SAME_PAUSE"}:
                    early_exit_banned_symbols.add(symbol)

                if reentry_mode == "BAN_SAME_PAUSE":
                    early_exits_since_pause += 1

                    if (
                        early_exits_since_pause
                        >= int(config.pause_after_early_exits_count)
                    ):
                        pause_minutes = float(
                            config.pause_new_entries_minutes
                        )
                        new_entry_pause_until = (
                            now + pd.Timedelta(minutes=pause_minutes)
                        )
        ...
        # --------------------------------------------------------
        pause_active = bool(
            new_entry_pause_until is not None
            and now < new_entry_pause_until
        )
        ...
        if (
            ...
            for _, row in latest_top5.iterrows():
                ...
                if (
                    reentry_mode in {"BAN_SAME", "BAN_SAME_PAUSE"}
                    and symbol in early_exit_banned_symbols
                ):
                    continue
    ...
    payload = {
        "ok": True,
        "version": "trade-replay-v7-reentry-control",
        "date": date_text,
...
def _reentry_strategy_configs() -> dict[str, ReplayTradeConfig]:
    """
    현재까지 유력한 조합을 고정하고 재진입 제어만 비교.

    고정:
    - BUY1 즉시 50%
    - BUY2 B_STRICT
    - 조기청산 C: WEAK60 + STAGNANT
    """
    common = dict(
        entry_initial_pct=50.0,
        entry_confirm_pct=0.0,
    ...
    return {
        "A_REENTRY_CURRENT": ReplayTradeConfig(
            **common,
            reentry_control_mode="NONE",
        ),
        "B_BAN_SAME_SYMBOL": ReplayTradeConfig(
            **common,
            reentry_control_mode="BAN_SAME",
        ),
        "C_BAN_SAME_PAUSE60": ReplayTradeConfig(
            **common,
            reentry_control_mode="BAN_SAME_PAUSE",
            pause_after_early_exits_count=2,
            pause_new_entries_minutes=60.0,
        ),
    }


def compare_reentry_control_strategies(
    dates: Iterable[str] | None = None,
    symbols: Iterable[str] | None = None,
) -> dict:
    """
    조기청산 후 재진입 제어 A/B/C 리플레이.
    실제 KIS 주문은 발생하지 않는다.
    """
    if dates is None:
        dates = ["2026-08-12"]
    ...
    dates = [str(x).strip() for x in dates if str(x).strip()]
    symbols = _normalize_symbols(symbols)
    strategies = _reentry_strategy_configs()
        ...
        for strategy_name, cfg in strategies.items():
            try:
                ...
                early_exit_count = sum(
                    1
                    for e in events
                    if str(e.get("액션", "")).startswith("EARLY_EXIT_")
                )
    ...
    stamp = datetime.now(ET).strftime("%Y%m%d_%H%M%S")
    daily_path = REPLAY_DIR / f"reentry_abc_daily_{stamp}.csv"
    aggregate_path = REPLAY_DIR / f"reentry_abc_summary_{stamp}.csv"
    json_path = REPLAY_DIR / f"reentry_abc_{stamp}.json"
    ...
    payload = {
        "ok": True,
        "version": "reentry-control-abc-v1",
        "dates": dates,
        "universe_count": len(symbols),
        "entry_fixed": "BUY1 즉시 50%",
        "buy2_fixed": "B_STRICT",
        "exit_fixed": "WEAK60 + STAGNANT",
        "strategies": {
            "A_REENTRY_CURRENT": (
                "조기청산 후 현재처럼 재진입 허용"
            ),
            "B_BAN_SAME_SYMBOL": (
                "조기청산된 같은 종목은 당일 재매수 금지"
            ),
            "C_BAN_SAME_PAUSE60": (
                "B + 조기청산 2회마다 신규진입 60분 중지"
            ),
        },
        "aggregate": aggregate_df.to_dict("records"),
        "daily": daily_df.to_dict("records"),
        "recommended_by_replay": recommended,
        "warning": (
            "리플레이 결과는 실제 체결과 다를 수 있고 미래 수익을 "
            "보장하지 않음. 여러 날짜 및 모의매매 재검증 필요."
        ),
        "files": {
            "daily_csv": str(daily_path),
            "aggregate_csv": str(aggregate_path),
